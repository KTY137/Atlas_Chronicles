...
for (const size of [64, 128]) {
  ...
  await sharp({ create: { width: 1660, height: cellHeight * 10, channels: 4, background: "#e4e8df" } }).composite(layers).png().toFile(join(out, `asset-contact-sheet-${size}.png`));
}
...
await writeFile(join(out, "asset-contact-sheet.html"), `<!doctype html><html lang="de"><meta charset="utf-8"><title>Zeitwelten · 100 Kartenassets</title><style>body{margin:32px;background:#edf0e8;color:#263b43;font:14px system-ui}h1{font-size:25px}header{position:sticky;top:0;background:#edf0e8;padding:12px 0;z-index:1}button,input{font:inherit;padding:8px;margin:0 8px 8px 0}main{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:16px}article{display:flex;align-items:center;flex-direction:column;background:#e4e8df;border:1px solid #c1ccc3;border-radius:9px;padding:12px;gap:7px}article img{width:128px;height:128px;object-fit:contain}article strong{font-size:12px}article small{font-size:11px;line-height:1.4;text-align:center;color:#52676a}article span{font-size:12px}[hidden]{display:none!important}.compact img{width:64px;height:64px}</style><header><h1>Zeitwelten · 100 eigenständige Draufsichten</h1><p>pk.zeitwelten 1.0.0 · CC0 · Metall, Keramik, Glas und Holz · 64 Pixel je Rasterzelle</p><input aria-label="Assets filtern" placeholder="Asset, Kategorie oder Tag suchen" oninput="for(const card of document.querySelectorAll('article'))card.hidden=!card.dataset.search.includes(this.value.toLowerCase())"><button onclick="document.querySelector('main').classList.toggle('compact')">64 / 128 Pixel</button><a href="asset-floor-tiles-4x4.png">Böden in 4 × 4 Kachelung</a></header><main>${cards}</main></html>\n`, "utf8");
console.log(`Contact sheets: ${out} (100 assets at 64/128 px; ${floors.length} floors tiled 4 × 4)`);
