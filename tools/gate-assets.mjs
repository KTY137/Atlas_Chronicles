...
const ERZEUGER = [
  ...
  { paket: "pk.genres", skript: "tools/assets/erzeuge-genrepaket.mjs" },
  { paket: "pk.natur", skript: "tools/assets/erzeuge-naturpaket.mjs" },
];

/** Erzeugtes Bildmaterial ausserhalb der Assetpakete; dieselbe Pflicht, sich zu reproduzieren. */
const ERZEUGTE_BILDER = [
...
async function main() {
  ...
  for (const { was, skript } of ERZEUGTE_BILDER) {
    // Nur pruefen, wo es das Verzeichnis gibt. Der synthetische Prueffixture von
    // `tools/test/gate-assets-binary.test.mjs` stellt das echte Gate ohne `packages/` und ohne
    // `assets/loot` auf; dort ist der Erzeuger nicht ausfuehrbar und auch nicht gemeint. Dass die
    // Bilder ueberhaupt dasind, haelt `packages/client/test/chronicle-loot-deck.test.ts` fest.
    try { await stat(join(ROOT, was)); } catch { continue; }
    try { await stat(join(ROOT, skript)); }
    catch { fehler.push(`${skript}: author script for ${was} is missing`); continue; }
    try { await promisify(execFile)(process.execPath, ["--import", "tsx", join(ROOT, skript), "--pruefe"], { cwd: ROOT }); }
    catch (error) { fehler.push(`${skript} --pruefe failed; ${was} no longer reproduces from its source:
${String(error.stdout ?? "")}${String(error.stderr ?? "")}`.trimEnd()); }
  ...
  console.log(`gate:assets GREEN — ${pakete.length} pack(s), ${assets} assets, ${verweise} resolvable references, ${ERZEUGER.length + ERZEUGTE_BILDER.length} reproduced from source`);
}
