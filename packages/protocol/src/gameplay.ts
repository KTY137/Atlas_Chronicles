...
const scalar = Type.Union([Type.String({maxLength:4000}),Type.Number({minimum:-1e12,maximum:1e12}),Type.Boolean()]);
const values = Type.Record(Type.String({minLength:1,maxLength:80}),scalar,{maxProperties:128});
export const PackageActivation = Type.Object({packageId:id,packageVersion:id,expectedVersion:Type.Integer({minimum:0})},closed);
