...
import { parseCampaignBundle, parseCampaignBundleV2, parseCampaignBundleV3, CAMPAIGN_BUNDLE_LIMITS, CAMPAIGN_BUNDLE_V3_LIMITS } from "@chronicle/io";
...
export async function runBundleCli(args: readonly string[], env: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  if (command !== "initialize" && command !== "check" && command !== "restore" && command !== "enroll")
    throw new CampaignRestoreError("Usage: campaign-bundle initialize|check|restore|enroll [--database-url URL] [--input FILE] [--upgrade-from-v1|--upgrade-from-v2] [--campaign-id ID --user-id ID]. DATABASE_URL is preferred.");
  const options = new Map<string, string>();
  let upgradeFromV1 = false, upgradeFromV2 = false;
  for (let index = 0; index < flags.length; index++) {
    const key = flags[index], value = flags[index + 1];
    if (key === "--upgrade-from-v1" || key === "--upgrade-from-v2") {
      if (upgradeFromV1 || upgradeFromV2 || (command !== "check" && command !== "restore")) throw new CampaignRestoreError("A source upgrade flag is a single explicit option for check or restore only.");
      upgradeFromV1 = key === "--upgrade-from-v1"; upgradeFromV2 = key === "--upgrade-from-v2"; continue;
    }
  ...
  if (file && (await stat(file)).size > (upgradeFromV1 || upgradeFromV2 ? CAMPAIGN_BUNDLE_LIMITS.bytes : CAMPAIGN_BUNDLE_V3_LIMITS.bytes)) throw new CampaignRestoreError("Campaign file exceeds the native bundle size limit.");
  const bundle = file ? (upgradeFromV1 ? parseCampaignBundle : upgradeFromV2 ? parseCampaignBundleV2 : parseCampaignBundleV3)(await readFile(file, "utf8")) : null;
  ...
  try {
    if (command === "initialize") {
      ...
      const result = command === "check" ? await inspectCampaignRestore(db, bundle, { upgradeFromV1, upgradeFromV2 }) : await restoreCampaignBundle(db, bundle, { upgradeFromV1, upgradeFromV2 });
