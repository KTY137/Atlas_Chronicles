...
export function registerWeek(app: FastifyInstance, db: Db, config: AppConfig) {
  ...
  app.post<{ Params: Entry }>("/api/campaigns/:campaignId/entries/:entryId/read", { schema: { body: empty } }, async req => week.markRead((await identity.authenticate(req.headers.cookie)).userId, req.params.campaignId, req.params.entryId));
