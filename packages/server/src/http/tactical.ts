...
export function registerTactical(app: FastifyInstance, db: Db, config: IdentityConfig) {
  ...
  app.post<{ Params: Scope; Body: P.TacticalImportInput }>(`${base}/tactical/maps/import-preview`, { bodyLimit: TACTICAL_IMPORT_BODY_LIMIT, schema: { body: P.TacticalImportSchema } }, req => run(async () => tactical.importPreview(await auth(req.headers.cookie), req.params.campaignId, req.body)));
  app.post<{ Params: Scope; Body: P.TacticalImportInput }>(`${base}/tactical/maps`, { bodyLimit: TACTICAL_IMPORT_BODY_LIMIT, schema: { body: P.TacticalImportSchema } }, req => run(async () => tactical.importMap(await auth(req.headers.cookie), req.params.campaignId, req.body)));
  ...
  app.put<{ Params: Item; Body: P.TacticalRevisionInput }>(`${base}/tactical/maps/:id/revision`, { bodyLimit: TACTICAL_IMPORT_BODY_LIMIT, schema: { body: P.TacticalRevisionSchema } }, req => run(async () => tactical.reviseMap(await auth(req.headers.cookie), req.params.campaignId, req.params.id, req.body)));
