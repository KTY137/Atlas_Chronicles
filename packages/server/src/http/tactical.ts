...
export function registerTactical(app: FastifyInstance, db: Db, config: IdentityConfig) {
  ...
  app.post<{ Params: Item; Body: P.TacticalFloorInput }>(`${base}/sessions/:id/tactical/floor`, { schema: { body: P.TacticalFloorSchema } }, req => run(async () => tactical.switchFloor(await auth(req.headers.cookie), req.params.campaignId, req.params.id, req.body)));
