...
export function registerGameplay(app: FastifyInstance, db: Db, config: AppConfig) {
  ...
  app.post<{Params:Scope;Body:Static<typeof P.RuleRuntimeEvaluation>}>(`${base}/rules/runtime/preview`, { schema: { body: P.RuleRuntimeEvaluation } },
  ...
  app.put<{Params:Item;Body:Static<typeof P.SheetUpdate>}>(`${base}/actors/:id/sheet`,{schema:{body:P.SheetUpdate}},async req=>{ const userId=await auth(req); if(req.body.packageContentHash){ const current=await game.getSheet(userId,req.params.campaignId,req.params.id); await ruleRuntime.assertContentHash(userId,req.params.campaignId,{id:current.packageId,version:current.packageVersion},req.body.packageContentHash); } return game.updateSheet(userId,req.params.campaignId,{expectedVersion:req.body.expectedVersion,fields:req.body.fields,actorId:req.params.id}); });
