...
export function registerGameplay(app: FastifyInstance, db: Db, config: AppConfig) {
  ...
  app.get<{Params:Scope;Querystring:Static<typeof P.RuleRuntimeSelection>}>(`${base}/rules/runtime`, { schema: { querystring: P.RuleRuntimeSelection } },
    async req => ruleRuntime.runtime(await auth(req), req.params.campaignId, { id: req.query.packageId, version: req.query.packageVersion }));
  app.post<{Params:Scope;Body:Static<typeof P.RuleRuntimeEvaluation>}>(`${base}/rules/runtime/preview`, { bodyLimit: P.RULE_BODY_LIMIT, schema: { body: P.RuleRuntimeEvaluation } },
  ...
  app.post<{Params:Scope;Body:unknown}>(`${base}/rules`,{bodyLimit:P.RULE_BODY_LIMIT,schema:{body:Type.Object({},{additionalProperties:true})}},async req=>game.installPackage(await auth(req),req.params.campaignId,req.body));
  app.post<{Params:Scope;Body:Static<typeof P.PackagePreview>}>(`${base}/rules/preview`,{bodyLimit:P.RULE_BODY_LIMIT,schema:{body:P.PackagePreview}},async req=>ruleActivation.previewPackage(await auth(req),req.params.campaignId,req.body.package));
  ...
  app.put<{Params:Item;Body:Static<typeof P.SheetUpdate>}>(`${base}/actors/:id/sheet`,{bodyLimit:P.RULE_BODY_LIMIT,schema:{body:P.SheetUpdate}},async req=>{ const userId=await auth(req); if(req.body.packageContentHash){ const current=await game.getSheet(userId,req.params.campaignId,req.params.id); await ruleRuntime.assertContentHash(userId,req.params.campaignId,{id:current.packageId,version:current.packageVersion},req.body.packageContentHash); } return game.updateSheet(userId,req.params.campaignId,{expectedVersion:req.body.expectedVersion,fields:req.body.fields,actorId:req.params.id}); });
