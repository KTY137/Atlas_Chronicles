...
describe("Der Zugang — die Adresse, die man begehen kann", () => {
  ...
  it("hält verknüpfte Räume beim Bearbeiten erreichbar und speichert weitere Änderungen regulär", async () => {
    ...
    const removedCartography = { ...map.cartography!, regions: map.cartography!.regions.filter(region => region.regionId !== room.knotenId) };
    await expect(tactical.reviseMap(gm, campaign, parentMapId, { schemaVersion: 2, cartography: removedCartography, addedBuildings: [], commandId: randomUUID(), expectedVersion: map.version, document: removed, anchors: map.anchors })).rejects.toBeInstanceOf(TacticalValidationError);
    const document = { ...map.document, environment: { ...map.document.environment, ambientLightArgb: "ff333333" } };
    await tactical.reviseMap(gm, campaign, parentMapId, { schemaVersion: 2, cartography: map.cartography, addedBuildings: [], commandId: randomUUID(), expectedVersion: map.version, document, anchors: map.anchors });
  ...
  it("platziert Zugänge innerhalb konkaver handgezeichneter Räume und behält deren Keim beim Editieren", async () => {
    ...
    const cartography = parseTacticalCartography({ ...map.cartography!, regions: [...map.cartography!.regions, { regionId: room.id, role: "room", authored: true, locked: false, provenance: null }] }, document);
    await tactical.reviseMap(gm, campaign, parentMapId, { schemaVersion: 2, cartography, addedBuildings: [], commandId: randomUUID(), expectedVersion: map.version, document, anchors: [] });
    ...
    await tactical.reviseMap(gm, campaign, parentMapId, { schemaVersion: 2, cartography, addedBuildings: [], commandId: randomUUID(), expectedVersion: list.version, document: updated, anchors: [] });
