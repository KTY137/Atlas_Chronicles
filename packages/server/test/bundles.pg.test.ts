...
import { campaignSemanticDiffV2 as campaignSemanticDiff, type CampaignBundleV2 as CampaignBundle } from "@chronicle/io";
...
describe.skipIf(!connection)("native campaign restore on real PostgreSQL", () => {
  ...
  it("checks and restores an explicit v1 input through the real local administrator CLI", async () => {
    ...
    try {
      ...
      expect(checked).toMatchObject({ formatVersion: 2, dryRun: true, migration: { sourceVersion: 1, targetVersion: 2 } });
      ...
      expect((await exportCampaignBundle(cliTarget, "gm", "campaign", config)).manifest.coreContentHash).toBe(checked.migration.sourceContentHash);
