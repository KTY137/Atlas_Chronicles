...
import { campaignSemanticDiff, type CampaignBundle } from "@chronicle/io";
...
const schemas = [0, 1, 2].map(() => `chronicle_bundle_${randomUUID().replaceAll("-", "")}`);
...
describe.skipIf(!connection)("native campaign restore on real PostgreSQL", () => {
  let admin: Db, source: Db, target: Db, failedTarget: Db, gm: string, campaign: string, entry: string, bundle: CampaignBundle;
  beforeAll(async () => {
    ...
    [source, target, failedTarget] = databases as [Db, Db, Db];
  ...
  afterAll(async () => {
    await Promise.all([source?.close(), target?.close(), failedTarget?.close()]);
    if (admin) {
      for (const schema of schemas) {
