...
it("refuses an occupied application port and closes its own SQL pool", async () => {
  ...
  try {
    ...
    await expect(startEmbeddedHost({ databaseUrl: "postgresql://chronicle:test@127.0.0.1:45002/postgres", origin: `http://localhost:${address.port}`, cookieSecret: "d".repeat(64), staticRoot: directory }, db)).rejects.toThrow();
