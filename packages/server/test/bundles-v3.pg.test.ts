...
import { createCampaignBundleV3, campaignSemanticDiffV3 } from "@chronicle/io";
...
describe.skipIf(!connection)("native v3 restoration on PostgreSQL", () => {
  ...
  beforeAll(async () => {
    ...
    [source, target, cli] = databases as [Db, Db, Db]; await restoreCampaignBundle(source, bundle);
  }, 30_000);
  afterAll(async () => {
  ...
  it("collects one tactical snapshot while a movement commits between state and token reads", async () => {
    ...
    expect(campaignSemanticDiffV3(bundle, await exporting)).toEqual([]);
  ...
  it("checks and restores v2 only through the explicit CLI upgrade flag", async () => {
    ...
    try {
      ...
      expect(checked).toMatchObject({ formatVersion: 3, dryRun: true, migration: { sourceVersion: 2, targetVersion: 3, steps: [{ algorithm: "atlas-chronicles/v2-to-v3/empty-tactical@1" }] } });
