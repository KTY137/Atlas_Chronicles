...
/**
 * 1.1.0 trug die Vitalwert-Deklaration. **1.2.0** ändert keine Spielmechanik, sondern beschreibt
 * erstmals die Bogenhierarchie explizit: Handeln, Wissen und Soziales liegen als Unterkategorien
 * unter Fähigkeiten; Ressourcen können ebenfalls gruppiert werden. Da auch Layout zum
 * Paketinhalt und damit zum Inhalts-Hash gehört, bekommt diese Änderung eine neue Version.
 */
export const HTBAH_VERSION = "1.2.0";
...
export function createHowToBeAHeroPackage(options: HtbahPackageOptions = {}): RulePackageV2 {
  ...
  const version = options.version ?? HTBAH_VERSION;
  const builtinMigration: readonly RuleMigration[] = options.id === undefined && options.version === undefined && options.skills === undefined && options.migrations === undefined
    ? [{ from: "1.1.0", to: HTBAH_VERSION, steps: [] }] : [];
  return parseRulePackageV2({ schemaVersion: 2, id: options.id ?? "de.howtobeahero.core", name: "How to be a Hero", version, engineVersion: "1.0.0", license: "CC-BY-NC-SA-4.0", authors: ["How to be a Hero-Team und Wiki-Beitragende", "Atlas Chronicles: deklarative Adaption und eigene Beispiele"], fields, layout: { sections: [
    { id: "character", label: "Figur und Absprachen", fields: ["name", "profession", "notes"] },
    { id: "skills", label: "Fähigkeiten", fields: [] },
    ...HTBAH_GROUPS.map(group => ({ id: group, label: HTBAH_GROUP_LABELS[group], parent: "skills", fields: skills.filter(skill => skill.group === group).flatMap(skill => [htbahSkillField(skill.id), htbahBonusField(skill.id)]) })),
    { id: "resources", label: "Ressourcen", fields: ["hp", "budget_adjustment"] },
    { id: "geistesblitz", label: "Geistesblitze", parent: "resources", fields: HTBAH_GROUPS.map(htbahSpentField) },
  ...
  ], attribution: HTBAH_ATTRIBUTION, migrations: options.migrations ?? builtinMigration, selfTests: [
