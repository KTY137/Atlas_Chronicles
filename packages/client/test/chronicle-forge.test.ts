...
describe("lossless supported rule workshop", () => {
  ...
  it("retains ordered outcomes, source notes, empty declarations and classification tests in forks", () => {
    const pkg = parseSupportedRulePackage({ ...CHRONICLE_HEROES_PACKAGE, computed: [], constraints: [], actions: CHRONICLE_HEROES_PACKAGE.actions.map(action => ({ ...action, preconditions: [] })) });
  ...
  it("blocks conflicting fixed thresholds and classification without discarding either draft value", () => {
    const draft = packageDraft(CHRONICLE_HEROES_PACKAGE); draft.actions[0]!.thresholdEnabled = true;
  ...
  it("checks classification expectations instead of treating an equal die total as a passed test", () => {
    const draft = packageDraft(CHRONICLE_HEROES_PACKAGE), first = draft.selfTests[0]!;
  ...
  it("shows derived values and classified results without threshold-up wording", () => {
    const pkg = CHRONICLE_HEROES_PACKAGE, fields = CHRONICLE_EXAMPLE_CHARACTERS[0]!.fields;
    const computed = renderToStaticMarkup(createElement(RuleComputedFields, { pkg, fields }));
    expect(computed).toContain("Talent · Körper"); expect(computed).toContain("vollständig verteilt");
    const receipt = evaluateSupportedAction(pkg, "talent_koerper", { seed: "00000001000000020000000300000004", actor: fields, input: {}, knowledge: { actorId: "actor", passages: [] } });
