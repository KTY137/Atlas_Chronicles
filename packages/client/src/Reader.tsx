...
function Text({ parts, onEntry, onDoor }: { parts: readonly Inline[]; onEntry: (id: string) => void; onDoor?: (id: string) => void }) {
  return <>{parts.map((part, i) => {
    ...
    for (const [j, mark] of part.marks.entries()) {
      ...
      else if (mark.art === "link") node = mark.tuer && onDoor ? <button key={j} className="article-link article-door" title={t("Vollmacht bis {zeitpunkt}", { zeitpunkt: new Date(mark.tuer.verfallAt).toLocaleString(locale()) })} onClick={() => onDoor(mark.tuer!.vollmachtId)}>{node}<span aria-hidden="true"> ↗</span></button> : mark.zielEntryId ? <button key={j} className="article-link" onClick={() => onEntry(mark.zielEntryId!)}>{node}</button> : <span key={j} className="unresolved-link" title={t("„{ziel}“ führt für dich noch zu keinem Artikel", { ziel: mark.zielSlug ?? "" })}>{node}</span>;
    }
...
export function BlockReader({ block, onEntry = () => {}, onDoor, campaignId }: { block: Block; onEntry?: (id: string) => void; onDoor?: (id: string) => void; campaignId?: string }) {
  switch (block.kind) {
    ...
    case "rohblock": return <div className="raw-block"><span>{t("Aus der Quelle übernommen · nicht umgewandelt")}</span><pre>{block.quelltext}</pre></div>;
  }
}
...
export function ArticleReader({ document, onEntry, members = [], onReveal, busy = false, campaignId, unreadIds, onSendPassage, onDoor }: {
  ...
  return <article className="article-body"><p className="eyebrow">{t("Chronik · {slug}", { slug: document.slug })}</p><h1>{document.titel}</h1>
    <p className="field-help">{gm ? t("Du liest als Spielleitung — du siehst hier mehr, als deine Spieler in ihrer Chronik zu sehen bekommen.") : t("Du liest mit dem Wissen deiner Figur — sichtbar ist nur, was sie bereits weiß.")}</p>
    {document.passagen.length === 0 ? <p className="muted">{gm ? t("Dieser Artikel hat noch keinen Inhalt.") : t("Deine Chronik weiß darüber noch nichts.")}</p> : null}
    {document.passagen.map((passage) => {
      ...
      return <section className={`passage${unreadIds?.has(passage.pid) ? " passage-unread" : ""}`} key={passage.pid} id={`passage-${passage.pid}`}>
        {unreadIds?.has(passage.pid) ? <span className="unread-label">{t("Neu für dich")}</span> : null}
        ...
        {onReveal ? <form className="reveal-control" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const actor = data.get("actor"); if (typeof actor === "string" && actor) onReveal(passage.pid, actor); }}>
          <label className="sr-only" htmlFor={`reveal-${passage.pid}`}>{t("Diese Passage für eine Figur freigeben")}</label>
          <select id={`reveal-${passage.pid}`} name="actor" required defaultValue="" disabled={busy || members.filter((m) => m.actorId).length === 0}><option value="" disabled>{t("Für eine Figur freigeben …")}</option>{members.filter((m) => m.actorId).map((m) => <option value={m.actorId!} key={m.userId}>{m.displayName}</option>)}</select>
          <Button type="submit" disabled={busy || !members.some((m) => m.actorId)}>{t("Passage freigeben")}</Button>
        </form> : null}
        {onSendPassage ? <Button variant="quiet" className="passage-letter" onClick={() => onSendPassage(passage.pid)}>{t("In einem Brief weitergeben")}</Button> : null}
