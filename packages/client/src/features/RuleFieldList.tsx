...
function FieldEditor({ field, index, length, sigil, onChange, onRemove, onMove }: { field: DraftField; index: number; length: number; sigil: string; onChange(value: DraftField): void; onRemove(): void; onMove(delta: -1 | 1): void }) {
  ...
  const noun = sigil === "?" ? "Parameter" : "Attribut";
  return <article className="rf-card"><div className="rf-section-heading"><h4>{field.label || `${noun} ${index + 1}`}</h4><span className="rf-toolbar"><OrderButtons index={index} length={length} onMove={onMove} /><Button variant="quiet" aria-label={`${noun} ${field.label} entfernen`} onClick={onRemove}><Trash2 size={15} /></Button></span></div>
    <div className="rf-form-grid"><label>Bezeichnung<input value={field.label} maxLength={120} required onChange={e => onChange({ ...field, label: e.target.value })} /></label><label>Kennung<input value={field.id} maxLength={96} required spellCheck={false} onChange={e => onChange({ ...field, id: e.target.value })} /><small>{`So heißt das Attribut in Formeln: In Formeln als ${sigil}${field.id || "kennung"}. Nur Kleinbuchstaben, Ziffern und „_", beginnend mit einem Buchstaben; ein Bindestrich lässt sich in Formeln nicht verwenden.`}</small></label><label>Typ<select value={field.type} onChange={e => onChange(changeFieldType(field, e.target.value as FieldSchema["type"]))}><option value="integer">Ganze Zahl</option><option value="number">Zahl mit Nachkommastellen</option><option value="string">Text</option><option value="boolean">Wahr / falsch</option></select></label>
    ...
    {idInvalid ? <Notice error>Die Kennung „{field.id}“ ist ungültig: Kleinbuchstaben, Ziffern, „_“ und „-“, das erste Zeichen muss ein Buchstabe sein. Beispiel: geschick.</Notice> : null}
