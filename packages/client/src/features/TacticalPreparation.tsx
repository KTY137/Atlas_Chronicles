...
export function TacticalPreparation({ campaignId, revision, onChanged, onDirty }: { campaignId: string; revision: number; onChanged: () => void; onDirty: (value: boolean) => void }) {
  ...
  const changeMode = (next: boolean) => {
    if (next === generating || (dirty && !window.confirm("Ungespeicherte Kartenänderungen verwerfen?"))) return;
  ...
  return <div className="tactical-preparation"><nav className="workbench-tabs" aria-label="Kartenbereiche">
    <Button aria-pressed={generating} onClick={() => changeMode(true)}>Neue Karte erzeugen</Button>
    <Button aria-pressed={!generating} onClick={() => changeMode(false)}>Kartenbibliothek · {maps.data?.length ?? 0}</Button>
  </nav>
    {canvasMenu && current.data && (!canvasMenu.nodeId || menuNode) ? <MapContextMenu key={canvasMenu.key} label={menuNode?.titel ?? current.data.name} popup={{...canvasMenu,onDismiss:()=>setCanvasMenu(null)}} actions={[
      {id:"open",label:menuNode ? "Unterkarte öffnen" : "Karte öffnen",disabled:!menuTarget,onSelect:()=>openMap({kind:"tactical",id:menuTarget!,name:menuNode?.titel ?? current.data!.name})},
      ...(menuTarget ? [{id:"delete",label:menuNode ? "Unterkarte löschen …" : "Karte löschen …",danger:true,onSelect:()=>{if(guard())setDeleting({kind:"tactical",id:menuTarget});}}] : []),
    ]} /> : null}
    ...
    {maps.error || current.error ? <Notice error>{maps.error || current.error}<Button onClick={onChanged}>Erneut laden</Button></Notice> : null}
    {!generating ? current.loading ? <Loading /> : current.data ? <><MapEditor key={current.data.id} current={current.data} campaignId={campaignId} onChanged={onChanged} onDirty={mapDirty} onContextMenu={canvasContext} /><ScenePlan key={current.data.id} map={current.data} campaignId={campaignId} revision={revision} onChanged={onChanged} onDirty={planDirty} /></> : <EmptyState title="Eine Karte für euren nächsten Abend.">Wähle eine gespeicherte Karte oder erzeuge eine neue. Danach kannst du sie bearbeiten und für eine Szene vorbereiten.</EmptyState> : null}
  </div>;
}
...
function PlanForm({ campaignId, scene, map: availableMap, current, actors, onChanged, onDirty }: { campaignId: string; scene: SceneCard; map: TacticalMapCard; current: TacticalPlan | null; actors: ActorCard[]; onChanged: () => void; onDirty: (value: boolean) => void }) {
  ...
  return <form onSubmit={e => { e.preventDefault(); if (baseline && baseline.mapId !== map.id && !window.confirm("Die bisherige Szenenkarte und ihre vorbereiteten Positionen ersetzen?")) return; void task.run(async () => {
    ...
    {baseline?.unavailable === "map-deleted" ? <Notice error>Die bisherige Szenenkarte wurde gelöscht. Speichere diese Vorbereitung mit einer vorhandenen Karte, bevor du die Szene beginnst.</Notice> : null}
