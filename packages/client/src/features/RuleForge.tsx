...
/** Beschriftung und Beschreibung eines Reiters als Funktion, nicht als Tabelle: so sieht
 * `t` ein Zeichenkettenliteral, und ein Sprachwechsel erreicht auch diese Texte. */
function tabLabel(id: EditorTab): string {
  switch (id) {
    case "package": return t("Paket");
    case "fields": return t("Felder");
    case "sheet": return t("Bogen");
    case "actions": return t("Aktionen");
    case "computed": return t("Berechnungen");
...
function navigateTabs(event: KeyboardEvent<HTMLButtonElement>, current: EditorTab, onChange: (tab: EditorTab) => void) {
  ...
  if (next < 0) return; event.preventDefault(); const id = tabs[next]!; onChange(id); document.getElementById(`rf-tab-${id}`)?.focus();
}
function tabDescription(id: EditorTab): string {
  switch (id) {
    case "package": return t("Name, Version, Kennung und Lizenz – die Grunddaten dieses Regelpakets.");
    case "fields": return t("Die Werte jeder Figur, z. B. Kraft oder Geschick – die Bausteine für Bogen und Aktionen.");
    case "sheet": return t("Wie die Felder oben auf dem Charakterbogen angeordnet werden.");
    case "actions": return t("Die Würfe: Formel, Boni und Erfolgsschwelle jeder Aktion.");
    case "computed": return t("Zusätzliche, aus vorhandenen Feldern berechnete Werte und Bedingungen für einen gültigen Bogen (erweitertes Paketformat).");
    case "tests": return t("Feste Beispiele, die bei jeder Installation automatisch nachgerechnet werden.");
    case "migrations": return t("Wie vorhandene Charakterbögen beim Wechsel auf diese Version übernommen werden.");
  }
}
...
function explainValidationError(message: string): string {
  const rewrites: [RegExp, string][] = [
    ...
    [/^field: reversed range$/, t("Bei einem Feld ist das Minimum größer als das Maximum. Öffne den Reiter „Felder“ und stelle sicher, dass jedes Minimum kleiner oder gleich seinem Maximum ist.")],
    ...
    [/^formula: unknown (actor|input)\.(.+)$/, t("Die Formel verwendet ein Feld, das es nicht mehr gibt. Öffne den Reiter „Aktionen“, such den markierten Baustein und wähle dort ein vorhandenes Feld neu aus.")],
    [/dice forbidden$/, t("In einer Versionsmigration sind Würfel nicht erlaubt, nur Berechnungen aus dem bisherigen Zahlenwert. Entferne den Würfel-Baustein aus der Formel im Reiter „Migration“.")],
    ...
    [/^rename: identical fields$/, t("Umbenennen: Die neue Feldkennung muss sich von der bisherigen unterscheiden.")],
  ];
  ...
  return <div className="rule-forge">
    ...
    <div className="rf-workspace"><aside className="rf-catalog" aria-label={t("Installierte Regelpakete")}><div className="rf-section-heading"><h2><BookOpen size={18} />{t("Bibliothek")}</h2><Button variant="quiet" disabled={task.busy} onClick={refresh} aria-label={t("Paketbibliothek aktualisieren")}>↻</Button></div>
      ...
      {current ? <><div className="rf-section-heading"><div><h2>{current.name || t("Unbenanntes Regelpaket")}</h2><span className="rf-help">{editable ? dirty ? t("Ungespeicherter Entwurf") : t("Entwurf") : t("Installierte Version · schreibgeschützt")} · {current.version}</span></div><div className="rf-toolbar">
        ...
        <div className="rf-card" aria-label={t("Aufbau dieses Regelpakets")}><div className="rf-section-heading"><h3>{t("Aufbau dieses Pakets")}</h3><span className="rf-help">{t("Schritt {schritt} von {gesamt}", { schritt: tabs.indexOf(tab) + 1, gesamt: tabs.length })}</span></div><p className="rf-help">{tabDescription(tab)}</p><div className="rf-toolbar"><span className="rf-node-badge">{t("{anzahl} Felder", { anzahl: current.fields.length })}</span><span className="rf-node-badge">{t("{anzahl} Abschnitte", { anzahl: current.sections.length })}</span><span className="rf-node-badge">{t("{anzahl} Aktionen", { anzahl: current.actions.length })}</span><span className="rf-node-badge">{t("{anzahl} Migrationswege", { anzahl: current.migrations.length })}</span><span className="rf-node-badge">{t("{anzahl} Pakettests", { anzahl: current.selfTests.length })}</span></div></div>
        <div className="rf-tabs" role="tablist" aria-label={t("Regelpaket bearbeiten")}>{tabs.map((id, index) => <button key={id} type="button" role="tab" aria-selected={tab === id} aria-controls={`rf-panel-${id}`} id={`rf-tab-${id}`} tabIndex={tab === id ? 0 : -1} title={t("Schritt {schritt} von {gesamt}: {beschreibung}", { schritt: index + 1, gesamt: tabs.length, beschreibung: tabDescription(id) })} onKeyDown={event => navigateTabs(event, id, setTab)} onClick={() => setTab(id)}>{tabLabel(id)}{id === "tests" && current.selfTests.length ? ` (${current.selfTests.length})` : ""}{errorLocation?.tab === id ? <TriangleAlert size={12} aria-label={t("Betrifft vermutlich den aktuellen Fehler")} /> : null}</button>)}</div>
        <fieldset className="rf-editor-fields" disabled={tab !== "actions" && (!editable || task.busy)}><div role="tabpanel" id={`rf-panel-${tab}`} aria-labelledby={`rf-tab-${tab}`}>
          ...
          {tab === "fields" ? <FieldList title={t("Charakterfelder")} fields={current.fields} onChange={fields => edit({ ...current, fields, sections: current.sections.map(s => ({ ...s, fieldKeys: s.fieldKeys.filter(id => fields.some(f => f.localId === id)) })) })} /> : null}
          ...
          {tab === "tests" ? <><h3>{t("Pakettests")}</h3><p>{t("Speichere Beispiele aus der Testtafel als feste Erwartung. Bei der Installation werden alle enthaltenen Tests ausgeführt.")}</p>{!current.selfTests.length ? <p className="rf-help">{t("Noch keine Pakettests. Unten auf der Testtafel kannst du für jede Figur ein Beispiel speichern.")}</p> : <div className="rf-test-list">{current.selfTests.map((test, i) => <article className="rf-card" key={i}><div className="rf-section-heading"><h4>{test.name}</h4><Button variant="quiet" aria-label={t("Pakettest {name} entfernen", { name: test.name })} onClick={() => edit({ ...current, selfTests: current.selfTests.filter((_, n) => n !== i) })}><Trash2 size={15} />{t("Entfernen")}</Button></div><p>{test.actionId} · {t("Erwartet {wert}", { wert: test.expectedTotal })} · {tests[i]?.passed ? t("Bestanden") : tests[i]?.error ?? (tests[i]?.actual === undefined ? t("Paket noch nicht gültig") : t("Ergebnis {wert} weicht ab", { wert: String(tests[i].actual) }))}</p><details><summary>{t("Gespeicherte Beispielwerte")}</summary><dl className="rf-value-list"><dt>{t("Würfelstart")}</dt><dd>{test.context.seed}</dd>{Object.entries(test.context.actor).map(([id, value]) => <div key={id}><dt>{id}</dt><dd>{String(value)}</dd></div>)}</dl><p>{t("{anzahl} gehaltene Beispielpassagen", { anzahl: test.context.knowledge.passages.length })}</p></details></article>)}</div>}</> : null}
          {tab === "migrations" ? <MigrationEditor draft={current} packages={packages} onChange={migrations => edit({ ...current, migrations })} /> : null}
        </div></fieldset>
        <div className="rf-validation" aria-live="polite">{validation && !validation.valid ? <Notice error><p>{explainValidationError(validation.error)}</p>{errorLocation && errorLocation.tab !== tab ? <Button variant="quiet" onClick={() => setTab(errorLocation.tab)}>{t("Zum Reiter „{reiter}“ springen", { reiter: errorLocation.label })}</Button> : null}</Notice> : <p><Check size={16} />{tests.length ? t("Paketstruktur, Feldtypen und Formeln gültig · {bestanden} von {gesamt} Pakettests bestanden.", { bestanden: tests.filter(pruefung => pruefung.passed).length, gesamt: tests.length }) : t("Paketstruktur, Feldtypen und Formeln gültig.")}</p>}{collision ? <Notice error>{t("Diese Kennung und Version sind bereits mit anderem Inhalt installiert. Wähle eine neue Version.")}</Notice> : null}</div>
        <div className="rf-publish"><h3>{t("Prüfen und übernehmen")}</h3><p>{t("Die Installation speichert eine unveränderliche Version. Die Aktivierung wechselt das Regelwerk dieser Runde und übernimmt die zuvor geprüften Feldänderungen.")}</p>
          ...
          {readyReview ? <Button variant="primary" disabled={task.busy || !exactInstalled || active || (!!readyReview.migration?.entities.length && !acknowledged)} onClick={activate}>{t("Geprüfte Version für diese Runde aktivieren")}</Button> : null}
          {readyReview && !exactInstalled ? <p className="rf-help">{t("Installiere zuerst genau diese geprüfte Version. Die Vorschau bleibt für die anschließende Aktivierung erhalten.")}</p> : null}
        </div>
...
function PackageEditor({ draft, onChange }: { draft: RuleDraft; onChange(value: RuleDraft): void }) {
  return <><h3>{t("Paketangaben")}</h3><p className="rf-help">{t("Diese Angaben identifizieren das Regelpaket als Ganzes, unabhängig davon, welche Felder und Aktionen es enthält.")}</p><div className="rf-form-grid"><label>{t("Name")}<input value={draft.name} maxLength={120} required placeholder={t("z. B. Mein Regelwerk")} onChange={e => onChange({ ...draft, name: e.target.value })} /><small>{t("Der Titel, den Spielleitung und Spielende in der Bibliothek sehen.")}</small></label><label>{t("Version")}<input value={draft.version} placeholder="1.0.0" required onChange={e => onChange({ ...draft, version: e.target.value })} /><small>{t("Drei durch Punkte getrennte Zahlen (Hauptversion.Nebenversion.Fehlerbehebung), z. B. 1.0.0. Jede installierte Version ist danach unveränderlich.")}</small></label><label>{t("Paketkennung")}<input value={draft.id} maxLength={128} placeholder="de.meine-runde.regelwerk" required onChange={e => onChange({ ...draft, id: e.target.value })} /><small>{t("Namensraum mit Punkten oder Bindestrichen, ausschließlich Kleinbuchstaben und Ziffern, z. B. de.meine-runde.regelwerk.")}</small></label><label>{t("Lizenz")}<input value={draft.license} maxLength={120} required placeholder={t("z. B. MIT")} onChange={e => onChange({ ...draft, license: e.target.value })} /><small>{t("Unter welchen Bedingungen andere dieses Paket weiterverwenden dürfen, z. B. MIT oder Alle Rechte vorbehalten.")}</small></label></div>
    <h4>{t("Urheberschaft")}</h4><p className="rf-help">{t("Wer dieses Regelwerk verfasst hat. Mindestens eine Angabe ist erforderlich.")}</p>{draft.authors.map((author, i) => <div className="rf-inline" key={i}><label>{t("Autor:in {n}", { n: i + 1 })}<input value={author} maxLength={120} required placeholder={t("Name oder Tischname")} onChange={e => onChange({ ...draft, authors: draft.authors.map((a, n) => n === i ? e.target.value : a) })} /></label><Button disabled={draft.authors.length <= 1} variant="quiet" aria-label={t("Autor:in {n} entfernen", { n: i + 1 })} onClick={() => onChange({ ...draft, authors: draft.authors.filter((_, n) => n !== i) })}><Trash2 size={15} /></Button></div>)}<Button disabled={draft.authors.length >= 32} onClick={() => onChange({ ...draft, authors: [...draft.authors, ""] })}><Plus size={15} />{t("Weitere Urheberschaft")}</Button>
    <p className="rf-help">{t("Dieses Paket verwendet die deklarativen Regelbausteine der Engine {version}. Formeln können Werte berechnen und gehaltenes Wissen prüfen. Aktionen benötigen am Tisch eine menschliche Bestätigung.", { version: ENGINE_VERSION })}</p></>;
}
...
function FieldEditor({ field, index, length, onChange, onRemove, onMove }: { field: DraftField; index: number; length: number; onChange(value: DraftField): void; onRemove(): void; onMove(delta: -1 | 1): void }) {
  ...
  return <article className="rf-card"><div className="rf-section-heading"><h4>{field.label || t("Feld {n}", { n: index + 1 })}</h4><span className="rf-toolbar"><OrderButtons index={index} length={length} onMove={onMove} /><Button variant="quiet" aria-label={t("Feld {name} entfernen", { name: field.label })} onClick={onRemove}><Trash2 size={15} /></Button></span></div>
    <div className="rf-form-grid"><label>{t("Bezeichnung")}<input value={field.label} maxLength={120} required onChange={e => onChange({ ...field, label: e.target.value })} /></label><label>{t("Feldkennung")}<input value={field.id} maxLength={96} required spellCheck={false} onChange={e => onChange({ ...field, id: e.target.value })} /><small>{t("So heißt das Feld in Formeln, z. B. actor.geschick. Nur Kleinbuchstaben, Ziffern, „_“ und „-“, beginnend mit einem Buchstaben.")}</small></label><label>{t("Typ")}<select value={field.type} onChange={e => onChange(changeFieldType(field, e.target.value as FieldSchema["type"]))}><option value="integer">{t("Ganze Zahl")}</option><option value="number">{t("Zahl mit Nachkommastellen")}</option><option value="string">{t("Text")}</option><option value="boolean">{t("Wahr / falsch")}</option></select></label>
    ...
    {idInvalid ? <Notice error>{t("Die Feldkennung „{kennung}“ ist ungültig: Kleinbuchstaben, Ziffern, „_“ und „-“, das erste Zeichen muss ein Buchstabe sein. Beispiel: geschick.", { kennung: field.id })}</Notice> : null}
...
function SheetEditor({ fields, sections, onChange }: { fields: DraftField[]; sections: DraftSection[]; onChange(sections: DraftSection[]): void }) {
  ...
  return <><div className="rf-section-heading"><h3>{t("Aufbau des Charakterbogens")}</h3><Button disabled={sections.length >= 32} onClick={() => onChange([...sections, { localId: localKey(), id: uniqueId("abschnitt", sections.map(s => s.id)), label: "Neuer Abschnitt", fieldKeys: [] }])}><Plus size={15} />{t("Abschnitt")}</Button></div><p className="rf-help">{t("Ordne Felder in Abschnitten an, z. B. „Werte“ oder „Ausrüstung“. Die Testtafel zeigt diesen Bogen unmittelbar mit Beispielwerten; Felder ohne Abschnitt erscheinen dort automatisch unter „Weitere Felder“.")}</p>
    {sections.map((section, index) => <article className="rf-card" key={section.localId}><div className="rf-section-heading"><h4>{section.label}</h4><span className="rf-toolbar"><OrderButtons index={index} length={sections.length} onMove={delta => onChange(moveItem(sections, index, delta))} /><Button variant="quiet" aria-label={t("Abschnitt {name} entfernen", { name: section.label })} onClick={() => onChange(sections.filter(s => s.localId !== section.localId))}><Trash2 size={15} /></Button></span></div><div className="rf-form-grid"><label>{t("Titel")}<input value={section.label} maxLength={120} onChange={e => update({ ...section, label: e.target.value })} /></label><label>{t("Abschnittskennung")}<input value={section.id} maxLength={96} onChange={e => update({ ...section, id: e.target.value })} /><small>{t("Nur zur internen Zuordnung, z. B. werte. Nur Kleinbuchstaben, Ziffern, „_“ und „-“, beginnend mit einem Buchstaben.")}</small></label></div>
      <ol className="rf-layout-fields">{section.fieldKeys.map((key, i) => <li key={key}><span>{fields.find(f => f.localId === key)?.label ?? t("Entferntes Feld")}</span><OrderButtons index={i} length={section.fieldKeys.length} onMove={delta => update({ ...section, fieldKeys: moveItem(section.fieldKeys, i, delta) })} /><Button variant="quiet" aria-label={t("Feld aus Abschnitt entfernen")} onClick={() => update({ ...section, fieldKeys: section.fieldKeys.filter(k => k !== key) })}><Trash2 size={14} /></Button></li>)}</ol>
      <label>{t("Feld zuordnen")}<select value="" onChange={e => { if (e.target.value) update({ ...section, fieldKeys: [...section.fieldKeys, e.target.value] }); }}><option value="">{t("Feld auswählen …")}</option>{fields.filter(f => !section.fieldKeys.includes(f.localId)).map(f => <option key={f.localId} value={f.localId}>{f.label} ({f.id})</option>)}</select></label>
    </article>)}</>;
}
...
function ActionEditor({ draft, disabled, onChange }: { draft: RuleDraft; disabled: boolean; onChange(actions: DraftAction[]): void }) {
  ...
  let expression = ""; if (action) { try { expression = draftExpression(action); } catch { expression = t("Die Formel enthält noch unvollständige Bausteine."); } }
  return <><div className="rf-section-heading"><h3>{t("Aktionen")}</h3><Button disabled={disabled || draft.actions.length >= RULE_LIMITS.actions} onClick={() => { const next = newAction(draft.actions.map(a => a.id)); onChange([...draft.actions, next]); setSelected(next.localId); }}><Plus size={15} />{t("Aktion")}</Button></div>
    <p className="rf-help">{t("Eine Aktion ist ein Wurf, z. B. „1d20 plus Geschick, Erfolg ab 15“: Würfel, Boni aus Feldern und eine Erfolgsschwelle.")}</p>
    {action ? <><label>{disabled ? t("Aktion ansehen") : t("Aktion bearbeiten")}<select value={action.localId} onChange={e => setSelected(e.target.value)}>{draft.actions.map(a => <option key={a.localId} value={a.localId}>{a.name} ({a.id})</option>)}</select></label><fieldset className="rf-editor-fields" disabled={disabled}><div className="rf-section-heading"><h4>{action.name}</h4><span className="rf-toolbar"><OrderButtons index={draft.actions.indexOf(action)} length={draft.actions.length} onMove={delta => onChange(moveItem(draft.actions, draft.actions.indexOf(action), delta))} /><Button variant="quiet" onClick={() => onChange(draft.actions.filter(a => a.localId !== action.localId))}><Trash2 size={15} />{t("Aktion entfernen")}</Button></span></div>
      <div className="rf-form-grid"><label>{t("Name")}<input value={action.name} maxLength={120} onChange={e => update({ ...action, name: e.target.value })} /></label><label>{t("Aktionskennung")}<input value={action.id} maxLength={96} onChange={e => update({ ...action, id: e.target.value })} /><small>{t("Wird in Pakettests und Migrationen verwendet, z. B. angriff. Nur Kleinbuchstaben, Ziffern, „_“ und „-“, beginnend mit einem Buchstaben.")}</small></label><label>{t("Aktionsversion")}<input value={action.version} placeholder="1.0.0" onChange={e => update({ ...action, version: e.target.value })} /><small>{t("Eigene Version dieser Aktion, unabhängig von der Paketversion, z. B. 1.0.0.")}</small></label></div><label>{t("Erklärung am Tisch")}<textarea rows={3} value={action.disclosure} maxLength={1024} onChange={e => update({ ...action, disclosure: e.target.value })} /><small>{t("Erkläre insbesondere, wie das Wissen der handelnden Figur das Ergebnis beeinflusst.")}</small></label>
      <label className="rf-check"><input type="checkbox" disabled={!!action.outcome} checked={action.thresholdEnabled} onChange={e => update({ ...action, thresholdEnabled: e.target.checked })} />{t("Feste Erfolgsschwelle verwenden")}</label>{action.thresholdEnabled ? <label>{t("Erfolg ab Ergebnis")}<input type="number" value={action.threshold} step="any" onChange={e => update({ ...action, threshold: e.target.value })} /><small>{t("Das Würfelergebnis mit allen Boni muss mindestens diese Zahl erreichen, z. B. 15 bei einem W20-Wurf.")}</small></label> : null}{action.outcome ? <p className="rf-help">{t("Diese Aktion nutzt stattdessen die geordneten Ergebnisbereiche weiter unten.")}</p> : null}
      <details className="rf-input-editor"><summary>{t("Eingaben der Aktion ({anzahl})", { anzahl: action.inputs.length })}</summary><FieldList title={t("Aktionseingaben")} fields={action.inputs} onChange={inputs => update({ ...action, inputs })} /></details>
      <p className="rf-help">{t("Baue unten Baustein für Baustein die Formel für das Ergebnis, z. B. ein Würfel plus ein Charakterfeld.")}</p>
      ...
      <details open><summary>{t("Ergebnis als Text (zur Kontrolle)")}</summary><code className="rf-expression">{expression}</code><p className="rf-help">{t("So liest sich die oben gebaute Formel als Ausdruck, z. B. 1d20 + actor.geschick.")}</p></details><p className="rf-help">{t("Am Tisch bleibt die menschliche Bestätigung für jede Aktion erforderlich.")}</p>
    </fieldset></> : <p>{t("Noch keine Aktion. Eine Aktion beschreibt einen Wurf, z. B. „1d20 plus Geschick, Erfolg ab 15“: Würfel, Charakterwert und eine Erfolgsschwelle. Lege oben die erste Aktion an.")}</p>}</>;
}
...
function MigrationEditor({ draft, packages, onChange }: { draft: RuleDraft; packages: readonly RulePackage[]; onChange(migrations: DraftMigration[]): void }) {
  ...
  return <><div className="rf-section-heading"><h3>{t("Charakterfelder migrieren")}</h3><Button disabled={draft.migrations.length >= 64} onClick={() => onChange([...draft.migrations, { localId: localKey(), from: sources.find(p => !draft.migrations.some(m => m.from === p.version))?.version ?? "", steps: [] }])}><Plus size={15} />{t("Migrationsweg")}</Button></div>
    <p>{t("Jeder Weg beschreibt einen direkten Wechsel von einer früheren Version zu {version}. Schritte werden der Reihe nach angewendet. Entfernte Werte müssen ausdrücklich archiviert und neue Felder ausdrücklich hinzugefügt werden.", { version: draft.version || t("dieser Version") })}</p>
    <p className="rf-help">{t("Bei vorhandenen Charakterbögen ist auch für unveränderte Felder ein Weg ohne Schritte erforderlich. Ein Wechsel zu einer anderen Paketkennung kann vorhandene Bögen nicht übernehmen. Bereits gespeicherte Würfe behalten ihre ursprünglichen Regeln.")}</p>
    <datalist id="rf-migration-versions">{sources.map(p => <option key={p.version} value={p.version} />)}</datalist>
    {draft.migrations.map(migration => <article className="rf-card" key={migration.localId}><div className="rf-section-heading"><h4>{migration.from || t("Ausgangsversion wählen")} → {draft.version}</h4><Button variant="quiet" aria-label={t("Migrationsweg von {version} entfernen", { version: migration.from })} onClick={() => onChange(draft.migrations.filter(m => m.localId !== migration.localId))}><Trash2 size={15} /></Button></div><label>{t("Ausgangsversion")}<input value={migration.from} list="rf-migration-versions" placeholder="1.0.0" onChange={e => update({ ...migration, from: e.target.value })} /><small>{t("Die frühere Paketversion, aus der Bögen übernommen werden, z. B. 1.0.0, muss sich von {version} unterscheiden.", { version: draft.version || t("dieser Version") })}</small></label>
      {sources.find(p => p.version === migration.from) ? <p className="rf-help">{t("Ausgangsfelder: {ausgang}. Zielfelder: {ziel}.", { ausgang: Object.keys(sources.find(p => p.version === migration.from)!.fields).join(", ") || t("keine"), ziel: draft.fields.map(f => f.id).join(", ") || t("keine") })}</p> : <p className="rf-help">{t("Diese Ausgangsversion ist noch nicht in der Bibliothek. Die tatsächlichen Charakterwerte prüft die Aktivierungsvorschau.")}</p>}
      {!migration.steps.length ? <p>{t("Keine Feldänderungen. Vorhandene Werte bleiben erhalten, sofern sie vollständig zum Zielbogen passen.")}</p> : null}
...
function MigrationStepEditor({ step, index, length, onChange, onRemove, onMove }: { step: DraftMigrationStep; index: number; length: number; onChange(step: DraftMigrationStep): void; onRemove(): void; onMove(delta: -1 | 1): void }) {
  return <fieldset className="rf-migration-step"><legend>{index + 1}. {step.kind === "rename" ? t("Umbenennen") : step.kind === "add" ? t("Hinzufügen") : step.kind === "archive" ? t("Archivieren") : t("Zahl umrechnen")}</legend><div className="rf-toolbar"><OrderButtons index={index} length={length} onMove={onMove} /><Button variant="quiet" onClick={onRemove} aria-label={t("Migrationsschritt {n} entfernen", { n: index + 1 })}><Trash2 size={15} /></Button></div>
    {step.kind === "rename" ? <div className="rf-form-grid"><label>{t("Bisherige Feldkennung")}<input value={step.from} onChange={e => onChange({ ...step, from: e.target.value })} /><small>{t("Wie das Feld in der Ausgangsversion heißt.")}</small></label><label>{t("Neue Feldkennung")}<input value={step.to} onChange={e => onChange({ ...step, to: e.target.value })} /><small>{t("Wie das Feld im Reiter „Felder“ jetzt heißt.")}</small></label></div> : <label>{t("Feldkennung")}<input value={step.field} onChange={e => onChange({ ...step, field: e.target.value })} /><small>{step.kind === "archive" ? t("Muss in der Ausgangsversion vorhanden sein.") : t("Muss im Reiter „Felder“ dieser Version vorhanden sein.")}</small></label>}
    {step.kind === "add" ? <div className="rf-form-grid"><label>{t("Werttyp")}<select value={step.type} onChange={e => { const type = e.target.value as FormulaType; onChange({ ...step, type, value: type === "number" ? "0" : type === "boolean" ? "false" : "" }); }}><option value="number">{t("Zahl")}</option><option value="string">{t("Text")}</option><option value="boolean">{t("Wahr / falsch")}</option></select></label><label>{t("Wert für bestehende Charaktere")}{step.type === "boolean" ? <select value={step.value} onChange={e => onChange({ ...step, value: e.target.value })}><option value="false">{t("Falsch")}</option><option value="true">{t("Wahr")}</option></select> : <input type={step.type === "number" ? "number" : "text"} step="any" value={step.value} onChange={e => onChange({ ...step, value: e.target.value })} />}<small>{t("Diesen Wert erhalten Charaktere, die dieses neue Feld noch nicht haben.")}</small></label></div> : null}
    {step.kind === "numeric" ? <><p className="rf-help">{t("Das Charakterfeld „value“ bezeichnet den bisherigen Zahlenwert dieses Feldes.")}</p><FormulaBuilder label={t("Neuer Zahlenwert")} value={step.formula} fields={{ actor: { value: "number" }, input: {} }} allowDice={false} allowKnowledge={false} onChange={formula => onChange({ ...step, formula })} /></> : null}
  </fieldset>;
}
...
function ReviewPanel({ review, acknowledged, onAcknowledge, disabled }: { review: RuleReview; acknowledged: boolean; onAcknowledge(value: boolean): void; disabled: boolean }) {
  return <section className="rf-review" aria-label={t("Geprüfte Aktivierung")}><h4>{t("Vorschau: {von} → {nach}", { von: review.from.version, nach: review.to.version })}</h4><p>{review.from.id} → {review.to.id}</p>
    ...
    <p className="rf-help">{t("Ändern sich Paket, aktive Version oder Charakterwerte, ist vor der Aktivierung eine neue Vorschau erforderlich.")}</p>
  </section>;
}
