...
import { RuleForgeNav, sectionDescription, sectionLabel, type ForgeSection } from "./RuleForgeNav";
...
/** Die ausklappbare Bogen-Vorschau: gemerkt je Gerät; ohne Wahl offen, wenn das Fenster breit genug ist. */
function readLivePreview(): boolean {
  ...
  return typeof window !== "undefined" && typeof window.matchMedia === "function" && window.matchMedia("(min-width: 1600px)").matches;
}
...
interface ErrorLocation { section: ForgeSection }
export function locateValidationError(message: string, draft: RuleDraft): ErrorLocation | null {
...
export function explainValidationError(message: string): string {
  const rewrites: [RegExp, string][] = [
    ...
    [/^layout: category nesting too deep$/, t("Der Kategorienbaum ist zu tief verschachtelt. Reduziere ihn auf höchstens 16 Ebenen.")],
...
export function RuleForge({ campaign, authorName, onDirty, onActivated }: { campaign: Campaign; authorName: string; onDirty(dirty: boolean): void; onActivated?: () => void }) {
  ...
  if (view === "library" || !current) return <div className="rule-forge">
    ...
    <div className="rf-library">
      <section className="rf-catalog" aria-label={t("Installierte Regelpakete")}><div className="rf-section-heading"><h2><BookOpen size={18} />{t("Bibliothek")}</h2><Button variant="quiet" disabled={task.busy} onClick={refresh} aria-label={t("Paketbibliothek aktualisieren")}>↻</Button></div>
        ...
        <div className="rf-library-tools"><label className="rf-library-search"><Search size={15} aria-hidden="true" /><input type="search" value={libraryQuery} aria-label={t("Regelpaket suchen")} placeholder={t("Name, Kennung oder Version")} onChange={event => setLibraryQuery(event.target.value)} /></label>
        ...
        <div className="rf-catalog-grid">{sichtbar.map(item => <div className="rf-catalog-row" key={packageKey(item)}><button type="button" className={`rf-catalog-item ${resource.data && packageKey(item) === packageKey(resource.data.pin) ? "is-active" : ""}`} disabled={task.busy} onClick={() => selectPackage(item)} onContextMenu={event => { event.preventDefault(); event.stopPropagation(); oeffneMenu(item, event.clientX, event.clientY); }} onKeyDown={event => { if (event.key !== "ContextMenu" && !(event.shiftKey && event.key === "F10")) return; event.preventDefault(); event.stopPropagation(); const rand = event.currentTarget.getBoundingClientRect(); oeffneMenu(item, rand.left, rand.bottom + 4); }}><strong>{item.name}</strong><span>{stand(item).genommen ? t("{version} · Aus der Bibliothek genommen", { version: item.version }) : resource.data && packageKey(item) === packageKey(resource.data.pin) ? t("{version} · Aktiv in dieser Runde", { version: item.version }) : t("{version} · Installiert", { version: item.version })}</span><small>{item.id}</small></button><Button variant="quiet" className="rf-catalog-menu" disabled={task.busy} aria-label={t("Paket {name} {version} verwalten", { name: item.name, version: item.version })} aria-haspopup="menu" onClick={event => { const bounds = event.currentTarget.getBoundingClientRect(); oeffneMenu(item, bounds.left, bounds.bottom + 4); }}><MoreHorizontal size={17} /></Button></div>)}</div>
        {!packages.length ? <p>{t("Noch kein Paket geladen. Du kannst einen Entwurf anlegen oder eine Paketdatei öffnen.")}</p> : null}{packages.length && !verfuegbar.length ? <p>{t("Alle Pakete sind aus der Bibliothek genommen. Über den Schalter oben werden sie wieder sichtbar.")}</p> : null}{verfuegbar.length > 0 && !sichtbar.length ? <div className="rf-search-empty" role="status"><p>{t("Kein Regelpaket gefunden.")}</p><Button variant="quiet" onClick={() => setLibraryQuery("")}>{t("Suche zurücksetzen")}</Button></div> : null}
    ...
    {!packages.length && !current ? <EmptyState title={t("Leg dein erstes Regelpaket an.")} action={<div className="rf-toolbar"><Button onClick={() => begin(starterDraft(authorName, packages))}><Plus size={16} />{t("Neues Paket beginnen")}</Button><Button onClick={() => upload.current?.click()}><Upload size={16} />{t("Paketdatei öffnen")}</Button></div>}>{t("Ein Regelpaket bündelt Charakterfelder wie Kraft oder Geschick, den Aufbau des Charakterbogens und Aktionen zu einer festen Version. Eine Aktion ist ein Wurf wie „1d20 plus Geschick, Erfolg ab 15“: ein Würfel, ein Charakterwert und eine Zahl, ab der die Aktion gelingt. „Neues Paket beginnen“ legt genau so ein Beispiel an, das du danach frei umbaust.")}</EmptyState> : null}
  </div>;
  ...
  const jump = (target: ForgePathTarget) => { if (target === "start") setView("library"); else open(target === "preview" ? "try" : target); };
  ...
  const sheetPreview = previewPkg && fixtures[0] ? <><h4>{t("So sieht der Bogen am Tisch aus")}</h4><p className="rf-help">{t("Testfigur {name}. Werte lassen sich hier ausprobieren; sie gehören nur der Vorschau.", { name: fixtures[0].name })}</p><LiveSheet pkg={previewPkg} fixture={fixtures[0]} onChange={setFirstValues} /></> : <Notice>{t("Die Vorschau erscheint, sobald der Entwurf einmal gültig ist.")}</Notice>;
  ...
  return <div className="rule-forge" ref={bench}>
    ...
    <div className={`rf-bench${drawer ? " has-live" : ""}`}>
      <RuleForgeNav draft={current} current={section} warning={errorLocation?.section ?? (!testsPass ? "try" : null)} onChange={setSection} />
      <section className="rf-editor" aria-label={t("Regelpaket")}><FormulaExampleContext.Provider value={figure}>
        <p className="rf-tab-description">{sectionDescription(section)}</p>
        <fieldset className="rf-editor-fields" disabled={(section === "package" || section === "migrations") && lockedOut}><div role="tabpanel" id={`rf-panel-${section}`} aria-labelledby={`rf-tab-${section}`}>
          {section === "package" ? <><PackageEditor draft={current} onChange={edit} />{pkg ? <RuleCapabilityCard capabilities={describeRuleCapabilities(pkg)} /> : null}{pkg ? <RuleAttribution pkg={pkg} /> : null}{current.attribution ? <AttributionEditor value={current.attribution} onChange={attribution => edit({ ...current, attribution })} /> : null}</> : null}
          {section === "map" ? <RuleMap draft={current} disabled={lockedOut} onChange={edit} onOpen={tab => setSection(tab)} /> : null}
          {section === "fields" ? <FieldList title={t("Attribute")} fields={current.fields} disabled={lockedOut} onChange={fields => edit(syncSheetWithFields(renameFieldReferences({ ...current, fields, sections: current.sections.map(s => ({ ...s, fieldKeys: s.fieldKeys.filter(id => fields.some(f => f.localId === id)) })) }, current.fields), current.fields))} /> : null}
          ...
          {section === "actions" ? <RuleActionEditor draft={current} disabled={lockedOut} onChange={actions => edit({ ...current, actions })} /> : null}
          ...
          {section === "try" ? <>
            ...
            <RuleForgePreview pkg={previewPkg} fixtures={fixtures} onFixtures={setFixtures} onSaveTest={editable && pkg && !task.busy && current.selfTests.length < 64 ? test => edit({ ...current, includeSelfTests: true, selfTests: [...current.selfTests, test] }) : undefined} />
          ...
          {section === "publish" ? <div className="rf-publish" tabIndex={-1}><RuleForgePath state={pathState} onJump={jump} /><h3>{t("Prüfen und übernehmen")}</h3><p>{t("Die Installation speichert eine unveränderliche Version. Die Aktivierung wechselt das Regelwerk dieser Runde und übernimmt die zuvor geprüften Feldänderungen.")}</p><ol className="rf-publish-steps"><li><strong>{t("Installieren")}</strong><span>{t("legt diese Version unveränderlich in die Bibliothek. Am Tisch ändert sich noch nichts.")}</span></li><li><strong>{t("Aktivierung prüfen")}</strong><span>{t("zeigt, was sich für vorhandene Figuren ändern würde, ohne etwas zu verändern.")}</span></li><li><strong>{t("Aktivieren")}</strong><span>{t("macht die geprüfte Version zum Regelwerk dieser Runde.")}</span></li></ol><div className="rf-toolbar"><Button disabled={!pkg || !testsPass || collision || task.busy || active} onClick={runPreview}><FlaskConical size={16} />{readyReview ? t("Vorschau erneuern") : t("Aktivierung prüfen")}</Button><Button variant="primary" disabled={!pkg || !testsPass || collision || exactInstalled || task.busy} onClick={install}>{exactInstalled ? t("Version ist installiert") : t("Version installieren")}</Button></div>{active ? <p className="rf-help">{t("Diese Paketversion ist bereits aktiv.")}</p> : null}{readyReview ? <ReviewPanel review={readyReview} acknowledged={acknowledged} onAcknowledge={setAcknowledged} disabled={task.busy} /> : null}{readyReview ? <Button variant="primary" disabled={task.busy || !exactInstalled || active || (!!readyReview.migration?.entities.length && !acknowledged)} onClick={activate}>{t("Geprüfte Version für diese Runde aktivieren")}</Button> : null}{readyReview && !exactInstalled ? <p className="rf-help">{t("Installiere zuerst genau diese geprüfte Version. Die Vorschau bleibt für die anschließende Aktivierung erhalten.")}</p> : null}</div> : null}
        </div></fieldset>
      </FormulaExampleContext.Provider></section>
      {drawer ? <aside className="rf-live-drawer" aria-label={t("Live-Vorschau des Bogens")}><div className="rf-section-heading"><span /><Button variant="quiet" aria-label={t("Bogen-Vorschau schließen")} onClick={toggleLivePreview}><X size={15} /></Button></div>{sheetPreview}</aside> : null}
    </div>
    <div className="rf-statusbar" id="rf-status" aria-live="polite">
      <div className="rf-status-check">{validation && !validation.valid ? <><TriangleAlert size={16} aria-hidden="true" /><span>{explainValidationError(validation.error)}</span>{errorLocation && errorLocation.section !== section ? <Button variant="quiet" onClick={() => setSection(errorLocation.section)}>{t("Zum Reiter „{name}“ springen", { name: sectionLabel(errorLocation.section) })}</Button> : null}</> : collision ? <><TriangleAlert size={16} aria-hidden="true" /><span>{t("Diese Kennung und Version sind bereits mit anderem Inhalt installiert. Wähle eine neue Version.")}</span>{section !== "package" ? <Button variant="quiet" onClick={() => setSection("package")}>{t("Zum Reiter „{name}“ springen", { name: sectionLabel("package") })}</Button> : null}</> : <p><Check size={16} />{tests.length ? t("Paketstruktur, Feldtypen und Formeln gültig · {bestanden} von {gesamt} Pakettests bestanden.", { bestanden: tests.filter(test => test.passed).length, gesamt: tests.length }) : t("Paketstruktur, Feldtypen und Formeln gültig.")}</p>}</div>
      {next && !(next.id === "start" && editable) && nextTarget !== section ? <Button variant={next.id === "start" ? "quiet" : "primary"} onClick={() => jump(next.id)}>{t("Weiter: {schritt}", { schritt: next.title })}<ArrowRight size={14} /></Button> : null}
    </div>
  </div>;
}
