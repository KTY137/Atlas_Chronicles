...
// Constant sources for a numeric migration step's formula ("@value" only) — module-level so
// every render passes FormulaField the same `sources`/`fields` identity; a fresh literal on every
// keystroke would defeat its memoisation and re-evaluate the example on every draft edit.
// `label` ist ein Zugriff, kein gespeicherter Text: so bleibt die Kennung dieser Konstanten
// stabil (die Merkfunktion von FormulaField haengt daran) und ein Sprachwechsel erreicht sie doch.
...
function locateValidationError(message: string, draft: RuleDraft): ErrorLocation | null {
  ...
  if ((quoted && draft.fields.some(f => f.id === quoted)) || (labelled && draft.fields.some(f => f.label === labelled))) return { tab: "fields", label: tabLabel("fields") };
  if ((quoted && draft.actions.some(a => a.id === quoted || a.inputs.some(i => i.id === quoted))) || (labelled && draft.actions.some(a => a.inputs.some(i => i.label === labelled)))) return { tab: "actions", label: tabLabel("actions") };
  if (/^field\b/.test(message)) return { tab: "fields", label: tabLabel("fields") };
  return null;
}
...
function SheetEditor({ fields, sections, onChange }: { fields: DraftField[]; sections: DraftSection[]; onChange(sections: DraftSection[]): void }) {
  ...
  return <><div className="rf-section-heading"><h3>{t("Aufbau des Charakterbogens")}</h3><Button disabled={sections.length >= 32} onClick={() => onChange([...sections, { localId: localKey(), id: uniqueId("abschnitt", sections.map(s => s.id)), label: "Neuer Abschnitt", fieldKeys: [] }])}><Plus size={15} />{t("Abschnitt")}</Button></div><p className="rf-help">{t("Ordne Felder in Abschnitten an, z. B. „Werte“ oder „Ausrüstung“. Die Testtafel zeigt diesen Bogen unmittelbar mit Beispielwerten; Felder ohne Abschnitt erscheinen dort automatisch unter „Weitere Felder“.")}</p>
    {sections.map((section, index) => <article className="rf-card" key={section.localId}><div className="rf-section-heading"><h4>{section.label}</h4><span className="rf-toolbar"><OrderButtons index={index} length={sections.length} onMove={delta => onChange(moveItem(sections, index, delta))} /><Button variant="quiet" aria-label={t("Abschnitt {name} entfernen", { name: section.label })} onClick={() => onChange(sections.filter(s => s.localId !== section.localId))}><Trash2 size={15} /></Button></span></div><div className="rf-form-grid"><label>{t("Titel")}<input value={section.label} maxLength={120} onChange={e => update({ ...section, label: e.target.value })} /></label><label>{t("Abschnittskennung")}<input value={section.id} maxLength={96} onChange={e => update({ ...section, id: e.target.value })} /><small>{t("Nur zur internen Zuordnung, z. B. werte. Nur Kleinbuchstaben, Ziffern, „_“ und „-“, beginnend mit einem Buchstaben.")}</small></label></div>
      <ol className="rf-layout-fields">{section.fieldKeys.map((key, i) => <li key={key}><span>{fields.find(f => f.localId === key)?.label ?? t("Entferntes Attribut")}</span><OrderButtons index={i} length={section.fieldKeys.length} onMove={delta => update({ ...section, fieldKeys: moveItem(section.fieldKeys, i, delta) })} /><Button variant="quiet" aria-label={t("Attribut aus Abschnitt entfernen")} onClick={() => update({ ...section, fieldKeys: section.fieldKeys.filter(k => k !== key) })}><Trash2 size={14} /></Button></li>)}</ol>
      <label>{t("Attribut zuordnen")}<select value="" onChange={e => { if (e.target.value) update({ ...section, fieldKeys: [...section.fieldKeys, e.target.value] }); }}><option value="">{t("Attribut auswählen …")}</option>{fields.filter(f => !section.fieldKeys.includes(f.localId)).map(f => <option key={f.localId} value={f.localId}>{t("{name} ({kennung})", { name: f.label, kennung: f.id })}</option>)}</select></label>
    </article>)}</>;
}

/** Der Name eines Migrationsschritts als Funktion mit Literalen, damit `t` sie sieht. */
function stepName(kind: DraftMigrationStep["kind"]): string {
  switch (kind) {
    case "rename": return t("Umbenennen");
    case "add": return t("Hinzufügen");
    case "archive": return t("Archivieren");
    default: return t("Zahl umrechnen");
  }
}
...
function MigrationStepEditor({ step, index, length, onChange, onRemove, onMove }: { step: DraftMigrationStep; index: number; length: number; onChange(step: DraftMigrationStep): void; onRemove(): void; onMove(delta: -1 | 1): void }) {
  return <fieldset className="rf-migration-step"><legend>{t("{n}. {schritt}", { n: index + 1, schritt: stepName(step.kind) })}</legend><div className="rf-toolbar"><OrderButtons index={index} length={length} onMove={onMove} /><Button variant="quiet" onClick={onRemove} aria-label={t("Migrationsschritt {n} entfernen", { n: index + 1 })}><Trash2 size={15} /></Button></div>
    ...
    {step.kind === "numeric" ? <FormulaField label={t("Neuer Zahlenwert")} help={t("@value ist der bisherige Zahlenwert dieses Attributs, zum Beispiel @value * 2.")} value={draftExpression(step)} onChange={expression => onChange({ ...step, expression })} sources={MIGRATION_NUMERIC_SOURCES} fields={MIGRATION_NUMERIC_FIELDS} allowDice={false} allowKnowledge={false} /> : null}
  </fieldset>;
}
...
function ReviewPanel({ review, acknowledged, onAcknowledge, disabled }: { review: RuleReview; acknowledged: boolean; onAcknowledge(value: boolean): void; disabled: boolean }) {
  return <section className="rf-review" aria-label={t("Geprüfte Aktivierung")}><h4>{t("Vorschau: {von} → {nach}", { von: review.from.version, nach: review.to.version })}</h4><p>{t("{von} → {nach}", { von: review.from.id, nach: review.to.id })}</p>
