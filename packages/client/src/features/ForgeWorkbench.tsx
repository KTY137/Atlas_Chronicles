...
import { Button, EmptyState, Loading, Notice, TabPanel, Tabs, ViewIntro, confirmAction } from "@chronicle/ui";
import { apiPath, type Campaign } from "../api";
import { useResource } from "../hooks";
import { t } from "../i18n";
import { Inventory, ItemTemplates } from "./ActorWorkbench";
import { ActorTemplates } from "./ActorTemplatesHost";
import { InstantiateActor } from "./InstantiateActorHost";
...
export function ForgeWorkbench({ campaign, authorName, liveRevision, section, onSectionChange, onOpenMaps, onOpenTable, onOpenRound, onDirty, onChanged }: {
  campaign: Campaign; authorName: string; liveRevision: number; section?: ForgeSection;
  onSectionChange?: (section: ForgeSection) => void; onOpenMaps?: () => void; onOpenTable?: (actorId?: string) => void; onOpenRound?: () => void;
  ...
  const navigate = async (next: ForgeSection) => {
    if (next === active) return;
    if (dirty && !await confirmAction({ title: t("Ungespeicherten Werkstatt-Entwurf verwerfen?"), message: t("Was du in diesem Bereich geändert und noch nicht gespeichert hast, geht verloren."), confirmLabel: t("Verwerfen"), cancelLabel: t("Weiter bearbeiten"), danger: true })) return;
    changed(false); setLocalSection(next); onSectionChange?.(next);
  };
  const gehe = (next: ForgeSection) => void navigate(next);
  ...
  return <div className={`forge-workbench${current ? " forge-workbench-editing" : ""}`}>
    ...
    <nav className="forge-navigation" aria-label={t("Werkstätten")}>
      <Button aria-current={active === "overview" ? "page" : undefined} onClick={() => gehe("overview")}><LayoutDashboard size={16} aria-hidden="true" />{t("Übersicht")}</Button>
      {TOOLS.map(({ id, icon: Icon }) => <Button key={id} aria-current={active === id ? "page" : undefined} onClick={() => gehe(id)}><Icon size={16} aria-hidden="true" />{werkzeug(id).label}</Button>)}
    </nav>
    {active === "overview" ? <>
      <ErsteSchritte campaignId={campaign.id} revision={liveRevision} onSectionChange={gehe} onOpenRound={onOpenRound} onOpenTable={onOpenTable ? () => onOpenTable() : undefined} />
      <section className="forge-overview" aria-labelledby="forge-start"><div className="forge-section-heading"><h2 id="forge-start">{t("Regeln und Figuren zuerst")}</h2><p>{t("Lege fest, wie eure Welt funktioniert, und erschaffe die Figuren, die sie erleben.")}</p></div>
        <div className="forge-tool-grid forge-tool-grid-primary">{TOOLS.filter(({ id }) => id === "rules" || id === "actors").map(({ id, icon: Icon }, index) => { const { label, summary, action } = werkzeug(id); return <button type="button" key={id} className="forge-tool forge-tool-featured" onClick={() => gehe(id)}>
      ...
      <section className="forge-overview forge-overview-secondary" aria-labelledby="forge-session"><div className="forge-section-heading"><h2 id="forge-session">{t("Den Spielabend ausgestalten")}</h2><p>{t("Karten, Beute und Bilder ergänzen eure Runde.")}</p></div>
        <div className="forge-tool-grid">{TOOLS.filter(({ id }) => id === "loot" || id === "maps" || id === "media").map(({ id, icon: Icon }) => { const { label, summary, action } = werkzeug(id); return <button type="button" key={id} className="forge-tool forge-tool-secondary" onClick={() => gehe(id)}>
      ...
      {current && active !== "actors" ? <header className="forge-section-heading"><h2>{active === "rules" ? t("Regelschmiede") : current.label}</h2><p>{current.summary}</p></header> : null}
      <Suspense fallback={<Loading text={t("Werkstatt wird geladen …")} />}>
        ...
        {active === "actors" ? <FigureWorkshop key={campaign.id} campaignId={campaign.id} revision={liveRevision} onChanged={onChanged} onDirty={changed} onOpenLoot={() => gehe("loot")} onOpenTable={onOpenTable} /> : null}
        ...
        {active === "media" ? <WikiMedien campaignId={campaign.id} onClose={() => gehe("overview")} closeLabel={t("Zur Schmiede")} embedded onDirty={changed} /> : null}
...
function LootWorkshop({ campaignId, revision, onChanged, onDirty }: { campaignId: string; revision: number; onChanged: () => void; onDirty: (dirty: boolean) => void }) {
  ...
  const choose = async (next: typeof tab) => { if (next === tab) return; if (dirty && !await verwerfenFrage()) return; report(false); setTab(next); };
  return <div className="actor-workbench">
    <Tabs label={t("Lootkarten bearbeiten")} idPrefix="lootkarten" className="forge-task-tabs" value={tab} onChange={next => void choose(next as typeof tab)}
      items={[{ id: "templates", label: t("Kartenvorlagen gestalten") }, { id: "inventory", label: t("Exemplare & Vorrat") }]} />
    <TabPanel idPrefix="lootkarten" id={tab} className="forge-task-panel">
    {tab === "templates" ? <ItemTemplates campaignId={campaignId} revision={revision + localRevision} onDirty={report} onChanged={refresh} onOpenInventory={() => void choose("inventory")} /> : <>
      ...
      {!actors.data && actors.error ? null : !actors.data && actors.loading ? <Loading /> : <Inventory campaignId={campaignId} actorId="" actors={actors.data ?? []} gm revision={revision + localRevision} onChanged={refresh} onDirty={report} onCreateTemplate={() => void choose("templates")} />}
    </>}
    </TabPanel>
  </div>;
}

function FigureWorkshop({ campaignId, revision, onChanged, onDirty, onOpenLoot, onOpenTable }: { campaignId: string; revision: number; onChanged: () => void; onDirty: (dirty: boolean) => void; onOpenLoot: () => void; onOpenTable?: (actorId?: string) => void }) {
  ...
  const report = useCallback((value: boolean) => { setDirty(value); onDirty(value); }, [onDirty]);
  const refresh = () => { setLocalRevision(value => value + 1); onChanged(); };
  // Die gewählte Vorlage wird im selben Zug gesetzt wie der Reiter: „Figur anlegen“ öffnet dann schon mit ihr.
  const choose = async (next: typeof tab, vorlage?: string) => {
    if (next === tab) return;
    if (dirty && !await verwerfenFrage()) return;
    report(false); if (vorlage !== undefined) setCreationTemplateId(vorlage); setTab(next);
  };
  return <div className="actor-workbench">
    <ViewIntro id="figuren-titel" title={t("Figuren & NPCs")} steps={[
      t("Lege eine Figurvorlage an: ein Muster mit Name, Art und Anfangswerten."),
      t("Lege daraus Figuren an und gib ihnen Namen und eigene Werte."),
      t("Gib Vorlagen für Spieler frei. Dann können sie unter „Ich“ selbst eine Figur beantragen."),
    ]}>{t("Hier entstehen die Figuren eurer Runde: aus einer Figurvorlage, von dir angelegt oder von Mitspielern beantragt.")} <Begriff id="figur" /></ViewIntro>
    <Tabs label={t("Figuren vorbereiten")} idPrefix="figuren" className="forge-task-tabs" value={tab} onChange={next => void choose(next as typeof tab)}
      items={[{ id: "templates", label: t("Figurvorlagen") }, { id: "create", label: t("Figur anlegen") }]} />
    {rules.error ? <Notice error>{rules.error}</Notice> : null}
    <TabPanel idPrefix="figuren" id={tab} className="forge-task-panel">
      {rules.data ? tab === "templates" ? <ActorTemplates campaignId={campaignId} rules={rules.data} revision={revision + localRevision} onChanged={refresh} onDirty={report} onOpenLoot={onOpenLoot} onInstantiate={templateId => void choose("create", templateId ?? "")} /> : <InstantiateActor campaignId={campaignId} rules={rules.data} initialTemplateId={creationTemplateId} revision={revision + localRevision} onChanged={refresh} onDirty={report} onCreateTemplate={() => void choose("templates")} onOpenTable={onOpenTable} ebene={3} /> : rules.loading && !rules.error ? <Loading /> : null}
    </TabPanel>
    ...
    {onOpenTable ? <p className="forge-context-link">{t("Vorhandene Figuren steuern, Bögen bearbeiten, Besitz verwalten und Anträge freigeben:")} <Button variant="quiet" onClick={() => onOpenTable()}>{t("Figuren am Tisch öffnen")}<ArrowRight size={14} aria-hidden="true" /></Button></p> : null}
  </div>;
}
