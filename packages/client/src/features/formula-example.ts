...
export function exampleContextFor(fields: readonly DraftField[], inputs: readonly DraftField[], figure: ExampleFigure | null, seed: string, actionId?: string): ExampleContext {
  return {
    seed, name: figure?.name || t("Beispielfigur"), actor: draftValues(fields, figure?.values ?? {}), input: draftValues(inputs, (actionId && figure?.inputs[actionId]) || {}),
...
const show = (value: Scalar): string => typeof value === "boolean" ? (value ? t("trifft zu") : t("trifft nicht zu")) : typeof value === "number" ? String(Math.round(value * 100) / 100).replace(/^-/, "−") : `„${value}“`;
function plain(message: string): string {
  if (/division by zero/.test(message)) return t("Mit den Beispielwerten teilt diese Formel durch null.");
  if (/unknown (actor|input)\./.test(message)) return t("Die Formel verweist auf ein Attribut, das die Beispielfigur nicht hat.");
  if (/limit/.test(message)) return t("Die Formel braucht mehr Rechenschritte, als erlaubt sind.");
  return t("Das Beispiel lässt sich mit diesen Werten nicht berechnen.");
}
export function formulaExample(canonical: string, context: ExampleContext, sources: FormulaSources): ExampleResult {
  try {
    ...
    const head = t("Beispiel für {name}: {wert}", { name: context.name, wert: show(result.value) });
