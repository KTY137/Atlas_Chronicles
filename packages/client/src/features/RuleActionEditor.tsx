...
/** Der Wurf in derselben Schreibweise wie im Formelfeld; eine unfertige Formel zeigt nichts. */
function rollText(action: DraftAction): string { try { return resugarFormula(draftExpression(action)); } catch { return ""; } }

export function RuleActionEditor({ draft, disabled, onChange, onExample }: { draft: RuleDraft; disabled: boolean; onChange(actions: DraftAction[]): void; onExample?: () => void }) {
  ...
  return <div className="rf-split">
    <nav className="rf-list" aria-label={t("Aktionen")}>
      ...
      <ul>{shown.map(a => <li key={a.localId}><button type="button" aria-current={a.localId === action?.localId ? "true" : undefined} onClick={() => setSelected(a.localId)}><strong>{a.name || t("Ohne Namen")}</strong><small>{rollText(a)}{a.inputs.length ? ` · ${plural(a.inputs.length, "{n} Parameter", "{n} Parameter")}` : ""}</small></button></li>)}</ul>
      {!draft.actions.length ? <ExampleStart text={t("Noch keine Aktion. Eine Aktion ist ein Wurf nach festen Regeln, zum Beispiel eine Probe auf Stärke: 1W20 plus Stärke, ab 15 geschafft.")} disabled={disabled} onExample={onExample} /> : null}
    </nav>
    {action ? <section className="rf-detail" aria-label={t("Aktion")}><fieldset className="rf-editor-fields" disabled={disabled}>
      ...
      <label>{t("Name")}<input value={action.name} maxLength={RULE_LIMITS.label} onChange={event => update({ ...action, name: event.target.value })} /><small>{t("So heißt die Aktion am Tisch, zum Beispiel Angriff.")}</small></label>
      <FieldList title={t("Parameter")} help={t("Was beim Würfeln abgefragt wird, zum Beispiel ein Bonus. In der Formel als ?kennung.")} fields={action.inputs} onChange={inputs => update({ ...action, inputs })} compact />
      <h5>{t("Ergebnis")}</h5>
      ...
      <details className="rf-input-editor rf-advanced"><summary>{t("Für Fortgeschrittene")}</summary>
        <label>{t("Kennung")}<input value={action.id} maxLength={96} spellCheck={false} onChange={event => update({ ...action, id: event.target.value })} /><small>{t('Für Pakettests und Migrationen, zum Beispiel angriff. Nur Kleinbuchstaben, Ziffern, „_" und „-", beginnend mit einem Buchstaben.')}</small></label>
        ...
        <label>{t("Erklärung am Tisch")}<textarea rows={3} value={action.disclosure} maxLength={RULE_LIMITS.longText} onChange={event => update({ ...action, disclosure: event.target.value })} /><small>{t("Erkläre, wie das Wissen der Figur das Ergebnis beeinflusst. Am Tisch bleibt die Bestätigung durch Menschen erforderlich.")}</small></label>
      </details>
