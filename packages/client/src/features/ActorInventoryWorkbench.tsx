...
function Reason({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <label>{t("Grund (freiwillig, erscheint im Verlauf)")}<input maxLength={500} value={value} onChange={event => onChange(event.target.value)} /></label>;
}
function RevisionPicker({ head, value, onChange }: { head: number; value: number; onChange: (value: number) => void }) {
  return <label>{t("Frühere Fassung ansehen")}<select value={value} onChange={event => onChange(Number(event.target.value))}>
    {Array.from({ length: head }, (_, index) => head - index).map(number => <option key={number} value={number}>{number === head ? t("Fassung {nummer} (aktuell)", { nummer: number }) : t("Fassung {nummer}", { nummer: number })}</option>)}
  </select></label>;
}
...
export function ItemTemplates({ campaignId, revision, onChanged, onDirty, onOpenInventory }: { campaignId: string; revision: number; onChanged: () => void; onDirty: (value: boolean) => void; onOpenInventory?: () => void }) {
  ...
  const choose = async (card: TemplateCard<ItemContract> | null) => { if (dirty && !await verwerfenFrage()) return; setSelected(card); setViewRevision(null); setSaved(false); reset(); };
  const pickRevision = async (number: number) => { if (dirty && !await verwerfenFrage()) return; setViewRevision(number); reset(); };
  ...
  return <div className="actor-columns actor-template-workspace"><section className="panel actor-template-library"><h2>{t("Deine Kartenvorlagen")}</h2><p className="field-help">{t("Gestalte eine Karte einmal und erzeuge daraus beliebig viele Exemplare. Bereits vergebene Karten behalten ihre bisherige Fassung.")}</p><Button onClick={() => void choose(null)}>{t("Neue Lootkarte")}</Button>
    ...
    <ul className="actor-object-list">{visible.map(card => <li key={card.id}><Button aria-pressed={selected?.id === card.id} onClick={() => void choose(card)}>{card.definition.name}<small>{t("{angabe} · Fassung {nummer}", { angabe: card.definition.schemaVersion === 2 ? seltenheitText(card.definition.seltenheit) : t("Ohne Kartengesicht"), nummer: card.revision })}</small></Button></li>)}</ul>
    {selected ? <RevisionPicker head={selected.revision} value={viewRevision ?? selected.revision} onChange={number => void pickRevision(number)} /> : null}
...
function ItemTemplateForm({ campaignId, original, onDirty, onSaved }: { campaignId: string; original: TemplateCard<ItemContract> | null; onDirty: (value: boolean) => void; onSaved: (archived?: boolean) => void }) {
  ...
  const sperre = !name.trim() ? t("Gib dem Gegenstand zuerst einen Namen.") : incompleteRows ? t("Ergänze in jeder begonnenen Zeile Bezeichnung und Wert oder entferne die Zeile.") : "";
  ...
  if (mediaOpen) return <section className="loot-media-editor"><p className="field-help">{t("Dein Kartenentwurf bleibt erhalten. Nach dem Hochladen kannst du das Bild auf deiner Karte auswählen.")}</p><WikiMedien campaignId={campaignId} embedded closeLabel={t("Zurück zur Lootkarte")} onDirty={setMediaDirty} onClose={() => {
    void (async () => {
      if (mediaDirty && !await confirmAction({ title: t("Datei verwerfen?"), message: t("Die ausgewählte Datei ist noch nicht hochgeladen und geht verloren."), confirmLabel: t("Verwerfen"), cancelLabel: t("Weiter hochladen"), danger: true })) return;
  ...
  return <div className="loot-editor-layout"><form className="panel loot-template-form" onChange={() => setFormDirty(true)} onSubmit={event => { event.preventDefault(); if (incompleteRows || !name.trim()) return; void task.run(async () => {
    await command(apiPath(campaignId, `/item-templates${original ? `/${original.id}` : ""}`), { definition: draft, ...(original ? { expectedVersion: original.version, reason: reasonOrDefault(reason, "vorlage") } : {}) }, original ? "PUT" : "POST"); setFormDirty(false); onDirty(false); onSaved();
    ...
    <div className="loot-save-actions"><Button type="submit" variant="primary" disabled={task.busy || !!sperre} aria-describedby={sperre ? sperrId : undefined}>{task.busy ? t("Wird gespeichert …") : original ? t("Neue Fassung speichern") : t("Lootkarte speichern")}</Button>
      {sperre ? <p id={sperrId} className="field-help">{sperre}</p> : null}</div>
    {original ? <details className="actor-optional"><summary>{t("Lootkarte archivieren")}</summary><p className="field-help">{t("Vorhandene Gegenstände und frühere Fassungen bleiben erhalten.")}</p><Button variant="danger" disabled={task.busy} onClick={() => void (async () => {
      if (!await confirmAction({ title: t("Lootkarte archivieren?"), message: t("Vorhandene Gegenstände bleiben erhalten. Neue Exemplare entstehen daraus nicht mehr."), confirmLabel: t("Lootkarte archivieren"), danger: true })) return;
      void task.run(async () => {
        await command(apiPath(campaignId, `/item-templates/${original.id}/archive`), { expectedVersion: original.version, reason: reasonOrDefault(reason, "vorlage") }); setFormDirty(false); onDirty(false); onSaved(true);
      });
    })()}>{t("Lootkarte archivieren")}</Button></details> : null}
...
/**
 * Inventar einer Figur oder der Vorrat der Spielleitung.
 *
 * `waehlbar={false}` blendet die eigene Inventarwahl aus, wo die Seite schon eine Figur gewählt
 * hat („Ich“: ein Figurwähler oben steuert Bogen und Inventar). `mitGeld={false}` lässt den
 * Geldzähler weg, wenn der Bogen darüber ihn schon zeigt — Geld steht dann einmal. `leise`
 * nimmt den Knöpfen die Hervorhebung, wenn das Inventar unter einer anderen Hauptaufgabe steht.
 */
export function Inventory({ campaignId, actorId, actors, gm, revision, onChanged, onDirty, onCreateTemplate, waehlbar = true, mitGeld = true, leise = false }: {
  campaignId: string; actorId: string; actors: ActorCard[]; gm: boolean; revision: number; onChanged: () => void; onDirty: (value: boolean) => void; onCreateTemplate?: () => void;
  waehlbar?: boolean; mitGeld?: boolean; leise?: boolean;
}) {
  ...
  const [templateId, setTemplateId] = useState(""); const task = useTask(), command = useCommand(), vorlageHinweis = useId();
  const haupt = leise ? "default" as const : "primary" as const;
  ...
  const chooseHolder = async (value: string) => { if (value === selectedHolder) return; if (dirty && !await verwerfenFrage()) return; setSelectedHolder(value); setSelected(""); setDirtyParts({ item: false, money: false }); };
  const chooseItem = async (id: string) => { if (selected === id) return; if (!dirty || await verwerfenFrage()) { setSelected(id); report(false); } };
  return <section className="inventory-section"><div className="page-heading"><h2>{stock ? t("Vorrat der Spielleitung") : openActor ? t("Inventar · {name}", { name: openActor.name }) : t("Inventar")}</h2>
      {waehlbar ? <label className="inventar-wahl">{t("Inventar")}<select value={selectedHolder} onChange={event => void chooseHolder(event.target.value)}>
    ...
    {!stock && mitGeld ? <Geldzaehler key={selectedHolder} campaignId={campaignId} actorId={selectedHolder} gm={gm} revision={revision} onChanged={onChanged} onDirty={reportMoney} /> : null}
    ...
    {gm && !templates.loading && !templates.error && !templates.data?.length ? <div className="panel actor-empty-hint"><h3>{t("Zuerst eine Lootkarte gestalten")}</h3><p>{t("Speichere eine Kartenvorlage in der Schmiede. Danach kannst du hier ein Exemplar in den Vorrat oder in ein Figureninventar legen.")}</p>{onCreateTemplate ? <Button variant={haupt} onClick={onCreateTemplate}>{t("Lootkarte erstellen")}</Button> : null}</div> : null}
    {gm && templates.data?.length ? <form className="panel inventory-create" onSubmit={event => { event.preventDefault(); void (async () => {
      if (dirtyParts.item && !await verwerfenFrage()) return; const card = templates.data?.find(entry => entry.id === templateId); if (card) void task.run(async () => {
...
function Beuteuebergabe({ campaignId, items, quelle, actors, busy, onChanged }: {
  ...
  return <section className="panel beute-uebergabe">
    ...
    <div className="button-row"><label className="inventar-wahl">{t("Ziel")}<select value={choice} disabled={busy || task.busy} onChange={event => { setChoice(event.target.value); setReport(null); }}>
      ...
      <Button disabled={busy || task.busy || !plan.length} onClick={() => void transfer()}>{plan.length ? plural(plan.length, "{n} Stück an {ziel} übergeben", "{n} Stücke an {ziel} übergeben", { ziel: targetName }) : t("Nichts zu übergeben")}</Button>
    </div>
    {task.error ? <Notice error>{task.error}</Notice> : null}
...
function ItemEditor({ campaignId, current, actors, gm, onDirty, onChanged }: { campaignId: string; current: ItemCard; actors: ActorCard[]; gm: boolean; onDirty: (value: boolean) => void; onChanged: () => void }) {
  ...
  const task = useTask(), command = useCommand(), besitzHinweis = useId();
  const dirty = JSON.stringify(state) !== JSON.stringify(baseline.state) || holder !== (baseline.holderActorId ?? "") || !!reason;
  const grund = reasonOrDefault(reason, "inventar"), ungespeichert = JSON.stringify(state) !== JSON.stringify(baseline.state);
  const uebernehmen = async () => { if (!dirty || await verwerfenFrage()) replace(current); };
  const archivieren = async () => {
    if (!await confirmAction({ title: t("Gegenstand archivieren?"), message: t("Der Gegenstand verschwindet aus dem Inventar. Sein bisheriger Verlauf bleibt erhalten."), confirmLabel: t("Gegenstand archivieren"), danger: true })) return;
    void task.run(async () => { await command(apiPath(campaignId, `/items/${baseline.id}/archive`), { expectedVersion: baseline.version, reason: grund }); onDirty(false); onChanged(); });
  };
  ...
  return <form className="panel" onSubmit={event => { event.preventDefault(); void task.run(async () => {
    const saved = await command<ItemCard>(apiPath(campaignId, `/items/${baseline.id}`), { expectedVersion: baseline.version, state, reason: grund }, "PUT"); replace(saved); onChanged();
  }); }}><fieldset className="actor-command-fields" disabled={task.busy}><h3>{baseline.definition.name}</h3><p className="field-help">{t("Lootkarte, Fassung {nummer}", { nummer: baseline.template.revision })}{baseline.definition.tags.length ? ` · ${baseline.definition.tags.join(" · ")}` : ""}</p>
    <Lootkarte definition={baseline.definition} campaignId={campaignId} />
    {current.version > baseline.version ? <Notice tone="info">{t("Der Gegenstand wurde inzwischen geändert.")} <Button onClick={() => void uebernehmen()}>{t("Aktuellen Gegenstand übernehmen")}</Button></Notice> : null}
    ...
    {gm ? <fieldset><legend>{t("Besitz zuordnen")}</legend><label>{t("Träger")}<select value={holder} onChange={event => setHolder(event.target.value)}><option value="">{t("Vorrat der Spielleitung")}</option>{actors.map(actor => <option key={actor.id} value={actor.id}>{actor.name}</option>)}</select></label>
      <p id={besitzHinweis} className="field-help">{t("Die Zuordnung überträgt den gespeicherten Gegenstand. Ungespeicherte Änderungen zuerst speichern.")}</p>
      <Button disabled={task.busy || ungespeichert} aria-describedby={besitzHinweis} onClick={() => void task.run(async () => {
        const saved = await command<ItemCard>(apiPath(campaignId, `/items/${baseline.id}/custody`), { expectedVersion: baseline.version, holderActorId: holder || null, reason: grund }); replace(saved); onChanged();
      })}>{t("Besitz übertragen")}</Button><Button variant="danger" disabled={task.busy} onClick={() => void archivieren()}>{t("Gegenstand archivieren")}</Button>
    </fieldset> : null}
  </fieldset></form>;
}
