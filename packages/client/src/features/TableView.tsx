...
import { Kampftisch } from "./Kampftisch";
import { HostRuleActionFields } from "./HostRuleActionFields";
...
export function TableView({ campaign, userId, onOpenEntry, onDirty, openDoor, openTab, onOpenForge, liveRevision = 0, readerScope = "", blickActorId = null }: { openTab?: { tab: TableTab; request: number; actor?: string }; onOpenForge?: (section: ForgeSection) => void; readerScope?: string; blickActorId?: string | null; openDoor?: { id?: string; request: number }; liveRevision?: number; campaign: Campaign; userId: string; onOpenEntry: (id: string) => void; onDirty: (value: boolean) => void }) {
  ...
  return <section className="page-content table-page"><div className="page-heading"><div><p className="eyebrow">{t("Der gemeinsame Abend")}</p><h1>{t("Am Tisch")}</h1><p className="muted">{active ? `${active.name} · ${active.fictionDate}` : t("Noch keine Szene eröffnet.")}</p></div><div className="button-row">{gm && onOpenForge ? <Button onClick={() => onOpenForge("overview")}><Hammer size={16} /> {t("Loot & NPCs vorbereiten")}</Button> : null}<Button aria-label={t("Tisch aktualisieren")} onClick={refresh}><RefreshCw size={16} /></Button></div></div>
    ...
    }}>{label}</button>)}</div>{controlled.length ? <label className="actor-picker">{t("Handelnde Figur")}<select value={actorId} onChange={(e) => { if (e.target.value === actorId) return; if (!dirty || window.confirm(t("Ungespeicherte Änderungen verwerfen?"))) { setActor(e.target.value); changeDirty(false); } else { changeDirty(dirty); } }}>{!actorId ? <option value="">{t("Figur wählen")}</option> : null}{controlled.map((member) => <option key={member.actorId} value={member.actorId!}>{member.displayName}</option>)}</select></label> : null}</div>
...
function Actions({ campaignId, actorId, rules, gm, fictionDate, roster, revision, onChanged }: { campaignId: string; actorId: string; rules: RulesState; gm: boolean; fictionDate: string; roster: Member[]; revision: number; onChanged: () => void }) {
  ...
  const action = runtime?.actions.find(candidate => candidate.id === actionId) ?? runtime?.actions[0];
  const actionState = action ? host.preview?.actionStates.find(candidate => candidate.actionId === action.id) : undefined;
  ...
  const sourceNames = new Map([...(runtime?.abilities ?? []), ...(runtime?.conditions ?? [])].map(source => [source.id, source.name]));
  return <div className="table-columns"><div className="action-column">
    ...
    <OffeneErleichterungen campaignId={campaignId} actorId={actorId} runtime={runtime} gm={gm} revision={revision}
      onChanged={onChanged} onGewuerfelt={() => onChanged()} />
    <form className="panel action-form" onSubmit={(event) => { event.preventDefault(); if (action && host.canSave) void task.run(async () => { await command(apiPath(campaignId, "/rolls"), { actorId, actionId: action.id, input: { ...defaults(action.inputs), ...input }, ...(passageId && gm ? { targetPassageId: passageId } : {}), ...(fictionDate ? { fictionDate } : {}) }); onChanged(); }); }}><h2><Dice6 size={20} /> {t("Eine Handlung wagen")}</h2>
    {task.error || sheet.error || host.error ? <Notice error>{task.error || sheet.error || host.error}</Notice> : null}{sheet.loading || host.pending && !runtime ? <Loading /> : action ? <><label>{t("Aktion")}<select value={action.id} onChange={(e) => { setActionId(e.target.value); setInput({}); }}>{runtime?.actions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><HostRuleActionFields action={action} state={actionState} input={input} onChange={setInput} disabled={task.busy || !host.canSave} />{gm ? <details><summary>{t("Erfolg mit einer Passage verbinden")}</summary><p className="field-help">{t("Bei Erfolg und deiner Bestätigung wird diese Passage am Tisch geprägt.")}</p><TargetPassage campaignId={campaignId} entryId={entryId} passageId={passageId} onEntry={setEntryId} onPassage={setPassageId} /></details> : null}<Button type="submit" variant="primary" disabled={task.busy || !host.canSave}>{task.busy ? <><span className="wuerfel-rollt" aria-hidden="true"><Dice6 size={17} /></span> {t("Würfelt …")}</> : <><Dice6 size={17} /> {t("Würfeln")}</>}</Button></> : <Notice error>{t("Keine Aktion für diesen Bogen verfügbar.")}</Notice>}
  </form>
    {gm ? <ErleichterungGewaehren campaignId={campaignId} rules={rules} roster={roster} revision={revision} onChanged={onChanged} /> : null}
  </div><section className="roll-feed"><h2>{t("Ergebnisse und Belege")}</h2>{rolls.loading ? <Loading /> : rolls.error ? <Notice error>{rolls.error}</Notice> : rolls.data?.length ? rolls.data.map((card) => <RollCard key={card.id} card={card} frisch={frisch.has(card.id)} namen={sourceNames} campaignId={campaignId} actorName={roster.find((member) => member.actorId === card.actorId)?.displayName ?? t("Figur")} onChanged={onChanged} />) : <EmptyState title={t("Der erste Wurf wartet.")}>{t("Ein echter Wurf kommt vom Server. Sein Rechenweg bleibt nachvollziehbar.")}</EmptyState>}</section></div>;
}
...
function Doors({ selectedId, campaignId, rules, roster, actorId, gm, revision, onChanged }: { selectedId?: string; campaignId: string; rules: RulesState; roster: Member[]; actorId: string; gm: boolean; revision: number; onChanged: () => void }) {
  ...
  const availableActions = runtime?.actions.filter(item => item.delegable) ?? [];
  ...
  return <div className="table-columns">{gm ? <form className="panel" onSubmit={(event) => { event.preventDefault(); if (action && host.canSave) void task.run(async () => { await command(apiPath(campaignId, "/vollmachten"), { actorId, passageId, actionId: action.id, input: { ...defaults(action.inputs), ...input }, threshold, expiresAt: Date.now() + hours * 3600_000, repeatable, budgetKind: budget, fictionDate: date }); onChanged(); }); }}><h2>{t("Eine Tür für später öffnen")}</h2><p className="field-help">{t("Die ausgewählte Figur kann die festgelegte Handlung selbst auslösen. Nur ihr wird diese Vollmacht angezeigt.")}</p><TargetPassage campaignId={campaignId} entryId={entryId} passageId={passageId} onEntry={setEntryId} onPassage={setPassageId} />
    {host.error ? <Notice error>{host.error}</Notice> : null}<label>{t("Aktion")}<select value={action?.id ?? ""} onChange={(e) => { setActionId(e.target.value); setInput({}); }} required><option value="">{t("Bitte wählen")}</option>{availableActions.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{action ? <HostRuleActionFields action={action} state={actionState} input={input} onChange={setInput} disabled={task.busy || !host.canSave} /> : null}
    {runtime?.actions.some(item => !item.delegable) ? <p className="field-help">{t("Proben mit Ergebnisbereichen werden am Tisch gewürfelt und bestätigt. Für diese Vollmacht stehen Aktionen mit einer festen Erfolgsschwelle zur Verfügung.")}</p> : null}
    <div className="rule-fields"><label>{t("Erfolg ab")}<input type="number" value={threshold} onChange={(e) => setThreshold(e.target.valueAsNumber)} required /></label><label>{t("Gültig für Stunden")}<input type="number" min={1} max={168} value={hours} onChange={(e) => setHours(e.target.valueAsNumber)} required /></label></div><label>{t("Datum in eurer Welt")}<input value={date} onChange={(e) => setDate(e.target.value)} required /></label><label>{t("Wochenkontingent")}<select value={budget} onChange={(e) => setBudget(e.target.value)}><option value="player">{t("Eine Vollmacht pro Spieler")}</option><option value="floating">{t("Freies Kontingent der Runde")}</option></select></label><label className="check-label"><input type="checkbox" checked={repeatable} onChange={(e) => setRepeatable(e.target.checked)} /> {t("Bei Misserfolg erneut versuchen dürfen")}</label><Button type="submit" variant="primary" disabled={task.busy || !actorId || !passageId || !action || !host.canSave}>{t("Vollmacht erteilen")}</Button>{task.error ? <Notice error>{task.error}</Notice> : null}
