...
/** Das Vergleichswort einer Ergebniszeile als Funktion mit Literalen, damit `t` sie sieht. */
function comparisonLabel(comparison: OutcomeComparison): string {
  switch (comparison) {
    case "eq": return t("gleich");
    case "lt": return t("kleiner als");
    case "lte": return t("höchstens");
    case "gt": return t("größer als");
...
function VitalEditor({ draft, onChange }: { draft: RuleDraft; onChange(draft: RuleDraft): void }) {
  ...
  return <section><h3>{t("Balken der Figur")}</h3>
    <p className="rf-help">{t("Leben, Mana, Ausdauer: eine Zahl auf dem Bogen, ein Höchststand und die Frage, was Erschöpfung bedeutet. Nur Zahlenattribute können Balken tragen.")}</p>
    ...
    {vitals.map((vital, i) => <fieldset className="rf-card" key={i}><legend>{vital.label || t("Balken {n}", { n: i + 1 })}</legend>
      <div className="rf-form-grid">
        <label>{t("Attribut auf dem Bogen")}<select value={vital.id} onChange={e => update(i, { id: e.target.value })}>
...
export function RuleDeclarativeEditor({ draft, onChange }: { draft: RuleDraft; onChange(draft: RuleDraft): void }) {
  ...
  return <><h3>{t("Abgeleitete Werte")}</h3><p className="rf-help">{t("Werte, die sich aus Attributen ergeben, zum Beispiel ein Bonus aus Geschick. Sie werden bei der Anzeige berechnet und nicht gespeichert.")}</p>
    {computed.map((value, i) => <fieldset className="rf-card" key={i}><legend>{value.label || t("Wert {n}", { n: i + 1 })}</legend><div className="rf-form-grid"><label>{t("Kennung")}<input value={value.id} onChange={e => changeComputed(i, { id: e.target.value })} /></label><label>{t("Beschriftung")}<input value={value.label} maxLength={120} onChange={e => changeComputed(i, { label: e.target.value })} /></label></div>
      <ExpressionInput label={t("Berechnung")} help={t("Ergibt sich aus Attributen, ohne Wurf.")} value={value.expression} onChange={expression => changeComputed(i, { expression })} draft={draft} />
    ...
    {outcome ? <><p>{t("Die erste passende Zeile entscheidet. Der eigentliche Wurf wird einmal ausgewertet; diese Vergleiche würfeln nicht erneut.")}</p>
      {outcome.bands.map((band, i) => {
        ...
        return <fieldset className="rf-card" key={i}><legend>{t("Bereich {n}: {ergebnis}", { n: i + 1, ergebnis: band.label })}</legend><div className="rf-form-grid"><label>{t("Kennung")}<input value={band.id} onChange={e => update({ id: e.target.value })} /></label><label>{t("Ergebnistext")}<input value={band.label} maxLength={120} onChange={e => update({ label: e.target.value })} /></label></div>
          <label htmlFor={`${controlId}-${i}`}>{t("Ergebnis vergleichen")}</label><select id={`${controlId}-${i}`} value={band.comparison} onChange={e => update({ comparison: e.target.value as OutcomeComparison })}>{(['eq', 'lt', 'lte', 'gt', 'gte'] as const).map(value => <option key={value} value={value}>{comparisonLabel(value)}</option>)}</select>
          <ExpressionInput label={t("Vergleichswert")} value={band.expression} onChange={expression => update({ expression })} draft={draft} action={action} />
