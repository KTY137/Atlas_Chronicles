...
export function MapEditor({ current, campaignId, onChanged, onDirty, onContextMenu, onOpenInterior }: { current: TacticalMapCard; campaignId: string; onChanged: () => void; onDirty: (value: boolean) => void; onContextMenu?: MapCanvasContext; onOpenInterior?: (nodeId: string, childId?: string | null) => void }) {
  ...
  const addPoint = (p: TacticalPoint) => {
    ...
    if (brush) {
      ...
      if (stamp && document.geometry.stamps.length < TACTICAL_MAP_LIMITS.stamps) {
        ...
        if (room?.locked) { setEditError("Dieser Raum ist gesperrt. Hebe die Sperre zum Einrichten auf."); return; }
  ...
  const computeGesture = () => {
    ...
    if (draft.stampId) {
      ...
      if (owner?.locked) { setEditError("Das zugehörige Gebäude oder der Raum ist gesperrt."); return; }
    ...
    if (interiorOperation) {
      if (!childrenConfirmed) { setEditError("Die Raumverbindungen werden noch geladen."); return; }
  ...
  const removeStamp = (id: string) => {
    if (selectedOwner?.locked) { setEditError("Der zugehörige Raum ist gesperrt."); return; }
  ...
  const toolHints: Record<MapToolSettings["tool"], string> = { select: "Anklicken zum Auswählen · Ziehen zum Verschieben · R drehen · Entf entfernen", terrain: "Gelände mit gedrückter Maustaste malen", road: "Einen Weg aufziehen · Anschlüsse entstehen beim Zeichnen", building: "Klicken zum Bauen · R dreht das Gebäude", room: "Raum aufziehen oder klicken, um die gewählte Vorlage zu setzen", wall: "Vom Anfang bis zum Ende ziehen, um eine Wand zu bauen", door: "Auf eine Wand klicken: Die Tür rastet in die Wand ein" };
  ...
  return <section className={`panel map-editor${sheet ? " map-editor-sheet" : ""}`} onKeyDown={event => {
    ...
    <div className="map-editor-workspace"><MapEditTools value={tools} onChange={changeTool} cellSize={cartography.construction.cellSize} onAssets={openAssets} assetsActive={!!brush}
      selected={selectedRegion} selectedTitle={selectedStamp ? selectedStamp.a.split("/").at(-1)?.replaceAll("_", " ") : interiorSelected ? interiorSelected.kind === "portal" ? "Tür" : "Wand" : selectedNode?.titel}
    ...
    <div className="map-editor-stage"><div className="map-editor-stage-toolbar"><span>{children.data?.art === "siedlung" ? "Außenkarte" : "Grundriss & Landschaft"}</span><div className="button-row"><Button aria-pressed={showGrid} onClick={() => setShowGrid(value => !value)}><Grid2X2 size={15} />Ansichtsraster</Button><Button onClick={openAssets}><Layers size={15} />Möbel & Objekte</Button></div></div>
    ...
    <div className="map-editor-hint"><MousePointer2 size={14} /><span>{brush ? "Objekt durch Klicken platzieren · R drehen · Esc beendet das Platzieren" : tools.hand ? "Ziehen zum Verschieben der Karte" : toolHints[tools.tool]}</span></div>
    {brush ? <div className="map-editor-stage-toolbar"><Button onClick={() => setBrushTurns(value => (value + 1) % 4)}><RotateCw size={15} /> Objekt drehen · {brushTurns * 90}°</Button><label>Objektgröße<select value={brushScale} onChange={event => setBrushScale(Number(event.target.value))}><option value={.5}>Halb</option><option value={1}>Normal</option><option value={1.5}>Groß</option><option value={2}>Doppelt</option></select></label><Button onClick={() => setBrush(null)}>Platzieren beenden</Button></div> : null}
    ...
    </div><div className="map-editor-details"><div className="map-editor-topline"><strong>Details & Einrichtung</strong><small>{document.geometry.regions.length} Flächen · {document.walls.length} Wände · {document.portals.length} Türen</small></div>
    ...
    {selectedNode ? <div className="tactical-object-selected"><strong>{selectedNode.titel}</strong>{selectedNode.bauwerk?.beschreibung ? <p>{selectedNode.bauwerk.beschreibung}</p> : null}
      {onOpenInterior ? <Button disabled={dirty || !childrenConfirmed} onClick={() => onOpenInterior(selectedNode.knotenId, selectedNode.vorhandeneKarteId)}>{selectedNode.vorhandeneKarteId ? "Innenraum bearbeiten" : "Innenraum anlegen"}</Button> : null}
      {dirty && onOpenInterior ? <p className="field-help">Speichere deinen Entwurf, um den Innenraum zu öffnen.</p> : null}
      {visible.addedRooms?.some(intent => intent.regionId === regionId) ? <label>Raumname<input maxLength={160} value={visible.addedRooms.find(intent => intent.regionId === regionId)?.titel ?? ""} onChange={event => { const titel = event.target.value; commit(old => ({ ...old, addedRooms: old.addedRooms?.map(intent => intent.regionId === regionId ? { ...intent, titel } : intent) })); }} /></label> : null}
    </div> : null}
    ...
    }}><strong>Raumgröße</strong><p className="field-help">Wände, Türen und zugehörige Einrichtung werden mitgeführt.</p><div className="map-numbers">{(["width", "height"] as const).map((name, axis) => {
      const coordinates = document.geometry.regions.find(region => region.id === regionId)!.punkte.map(p => p[axis]!);
      return <label key={name}>{axis ? "Raumtiefe in Zellen" : "Raumbreite in Zellen"}<input name={name} type="number" min={.5} step={.5} defaultValue={(Math.max(...coordinates) - Math.min(...coordinates)) / cartography.construction.cellSize} required /></label>;
    })}</div><Button type="submit" disabled={editingDisabled || selectedRegion.locked}>Raumgröße anwenden</Button></form> : null}
