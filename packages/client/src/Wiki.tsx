...
import { BookOpen, Columns2, History, Images, Network, Pencil, Plus, Search, RefreshCw, Upload } from "lucide-react";
...
export function Wiki({ campaign, onDirty, liveRevision = 0, onComposeLetter, onOpenDoors }: { campaign: Campaign; onDirty: (dirty: boolean) => void; liveRevision?: number; onComposeLetter?: (seed: { entryId: string; passageIds: string[] }) => void; onOpenDoors?: (id: string) => void }) {
  ...
  const select = (id: string | null) => {
    ...
    setSelected(id); setEditor(null); setHistory(null); setVergleich(false); setGefuege(false); setImporting(false); setMedien(false); changeDirty(false); setNotice(""); task.setError("");
  ...
  return <div className="wiki-layout"><aside className="entry-sidebar" aria-label="Artikelübersicht"><div className="sidebar-heading"><div><p className="eyebrow">Deine Welt</p><h2>Die Chronik</h2></div>{canEdit ? <Button aria-label="Artikel anlegen" title="Artikel anlegen" onClick={startNew}><Plus size={18} /></Button> : null}</div>
    ...
    {entries.loading ? <Loading text="Artikel werden geladen …" /> : entries.error ? <Notice error>{entries.error} <Button variant="quiet" onClick={() => setRevision((v) => v + 1)}>Erneut versuchen</Button></Notice> : <nav className="entry-list" aria-label="Wiki-Artikel">{entries.data?.map((entry) => <button key={entry.id} className={selected === entry.id ? "entry-item active" : "entry-item"} onClick={() => select(entry.id)} aria-current={selected === entry.id ? "page" : undefined}><BookOpen size={16} /><span><strong>{entry.title}</strong><small>{entry.excerpt || "Noch ohne Text"}</small></span></button>)}</nav>}
    ...
    {canEdit ? <div className="sidebar-import"><Button onClick={openImport}><Upload size={14} /> Wiki importieren</Button><Button onClick={openMedien}><Images size={14} /> Bilder</Button></div> : null}
