...
import { BookOpen, History, Images, Pencil, Plus, Search, RefreshCw, Upload } from "lucide-react";
...
export function Wiki({ campaign, onDirty, liveRevision = 0, onComposeLetter, onOpenDoors }: { campaign: Campaign; onDirty: (dirty: boolean) => void; liveRevision?: number; onComposeLetter?: (seed: { entryId: string; passageIds: string[] }) => void; onOpenDoors?: (id: string) => void }) {
  ...
  const select = (id: string | null) => {
    ...
    setSelected(id); setEditor(null); setHistory(null); setImporting(false); setMedien(false); changeDirty(false); setNotice(""); task.setError("");
  ...
  const startNew = () => { if (dirty && !window.confirm("Ungespeicherte Änderungen verwerfen?")) return; setEditor(newEditorSeed()); setHistory(null); setImporting(false); setMedien(false); };
