...
export function Wiki({ campaign, onDirty, liveRevision = 0, onComposeLetter, onOpenDoors }: { campaign: Campaign; onDirty: (dirty: boolean) => void; liveRevision?: number; onComposeLetter?: (seed: { entryId: string; passageIds: string[] }) => void; onOpenDoors?: (id: string) => void }) {
  ...
  return <div className="wiki-layout"><aside className="entry-sidebar" aria-label={t("Artikelübersicht")}><div className="sidebar-heading"><div><p className="eyebrow">{t("Deine Welt")}</p><h2>{t("Die Chronik")}</h2></div>{canEdit ? <Button aria-label={t("Artikel anlegen")} title={t("Artikel anlegen")} onClick={startNew}><Plus size={18} /></Button> : null}</div>
    ...
    {entries.loading ? <Loading text={t("Artikel werden geladen …")} /> : entries.error ? <Notice error>{entries.error} <Button variant="quiet" onClick={() => setRevision((v) => v + 1)}>{t("Erneut versuchen")}</Button></Notice> : geordnet ? <NavigationBaum daten={navigation.data!} ausgewaehlt={selected} onEntry={select} campaignId={canEdit ? campaign.id : undefined} onMoved={() => setRevision(value => value + 1)} /> : <nav className="entry-list" aria-label={t("Wiki-Artikel")}>{entries.data?.map((entry) => <button key={entry.id} className={selected === entry.id ? "entry-item active" : "entry-item"} onClick={() => select(entry.id)} aria-current={selected === entry.id ? "page" : undefined}><BookOpen size={16} /><span><strong>{entry.title}</strong><small>{entry.excerpt || t("Noch ohne Text")}</small></span></button>)}</nav>}
