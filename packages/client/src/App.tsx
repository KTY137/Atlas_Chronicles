...
import { BookOpen, ChevronDown, Compass, Dice6, LogOut, MessageSquare, Plus, Settings, Users } from "lucide-react";
...
type Stage = "wiki" | "atlas" | "tisch" | "kanal" | "runde" | "account";
const stages: { id: Stage; label: string; icon: typeof BookOpen }[] = [{ id: "wiki", label: "Chronik", icon: BookOpen }, { id: "atlas", label: "Atlas", icon: Compass }, { id: "tisch", label: "Tisch", icon: Dice6 }, { id: "kanal", label: "Kanal", icon: MessageSquare }, { id: "runde", label: "Runde", icon: Users }];
...
export function App() {
  ...
  const [stage, setStage] = useState<Stage>(() => { const value = new URLSearchParams(location.search).get("stage"); return value === "atlas" || value === "runde" || value === "account" || value === "tisch" || value === "kanal" ? value : "wiki"; });
  ...
  const navigate = (next: Stage) => {
    if (!guard()) return; setDirty(false); setStage(next); setChoosing(false);
    const url = new URL(location.href); url.searchParams.set("stage", next); window.history.replaceState(null, "", url);
  };
  const chooseCampaign = (id: string) => {
    if (!guard()) return; setDirty(false); setCampaignId(id); setChoosing(false); setStage("wiki");
  ...
  const openEntry = (id: string) => {
    ...
    window.history.replaceState(null, "", url); setDirty(false); setChoosing(false); setStage("wiki");
  };
  ...
  return <div className="application"><a className="skip-link" href="#main-content">Zum Inhalt</a><header className="context-bar"><button className="wordmark" onClick={() => { if (guard()) { setDirty(false); setChoosing(true); } }} aria-label="Kampagnenübersicht"><span aria-hidden="true">A✧</span><strong>ATLAS <small>CHRONICLES</small></strong></button><span className="context-divider" /><Button variant="quiet" className="campaign-trigger" onClick={() => { if (guard()) { setDirty(false); setChoosing(true); } }}>{campaign?.name ?? "Deine Kampagnen"}<ChevronDown size={15} /></Button><div className="context-right"><span className="user-name">{me.displayName}</span><Button variant="quiet" aria-label="Zugang verwalten" onClick={() => navigate("account")}><Settings size={18} /></Button><Button variant="quiet" aria-label="Abmelden" disabled={task.busy} onClick={() => { if (guard()) void task.run(async () => { await api("/api/logout", { method: "POST" }); setMe(null); setDirty(false); setCampaignId(""); }); }}><LogOut size={18} /></Button></div></header>
    <div className="app-main"><nav className="rail" aria-label="Bereiche">{stages.map(({ id, label, icon: Icon }) => <button key={id} className={stage === id && !choosing ? "rail-item active" : "rail-item"} aria-current={stage === id && !choosing ? "page" : undefined} onClick={() => navigate(id)} disabled={!campaign}><Icon size={21} /><span>{label}</span></button>)}</nav>
