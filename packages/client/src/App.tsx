...
import { BookOpen, CalendarDays, Hammer, ChevronDown, Compass, Dice6, LogOut, MessageSquare, Plus, Settings, Users } from "lucide-react";
...
const RuleForge = lazy(() => import("./features/RuleForge").then(module => ({ default: module.RuleForge })));
const MediaPanel = lazy(() => import("./features/MediaPanel").then(module => ({ default: module.MediaPanel })));
type Stage = "wiki" | "atlas" | "tisch" | "kanal" | "woche" | "schmiede" | "runde" | "account";
const stages: { id: Stage; label: string; icon: typeof BookOpen }[] = [{ id: "wiki", label: "Chronik", icon: BookOpen }, { id: "atlas", label: "Atlas", icon: Compass }, { id: "tisch", label: "Tisch", icon: Dice6 }, { id: "kanal", label: "Kanal", icon: MessageSquare }, { id: "woche", label: "Woche", icon: CalendarDays }, { id: "schmiede", label: "Schmiede", icon: Hammer }, { id: "runde", label: "Runde", icon: Users }];
...
export function App() {
  ...
  const [stage, setStage] = useState<Stage>(() => { const value = new URLSearchParams(location.search).get("stage"); return value === "atlas" || value === "runde" || value === "account" || value === "tisch" || value === "kanal" || value === "woche" || value === "schmiede" ? value : "wiki"; });
  ...
  const task = useTask(), campaigns = useResource<Campaign[]>(me ? "/api/campaigns" : null, revision);
  ...
  const chooseCampaign = (id: string) => {
    if (!guard()) return; setDirty(false); setCampaignId(id); setComposeSeed(undefined); setDoorRequest(undefined); setChoosing(false); setStage("wiki");
    const url = new URL(location.href); url.searchParams.set("campaign", id); url.searchParams.delete("entry"); url.searchParams.set("stage", "wiki"); window.history.replaceState(null, "", url);
  };
  ...
  return <div className="application"><a className="skip-link" href="#main-content">Zum Inhalt</a><header className="context-bar"><button className="wordmark" onClick={() => { if (guard()) { setDirty(false); setChoosing(true); } }} aria-label="Kampagnenübersicht"><span aria-hidden="true">A✧</span><strong>ATLAS <small>CHRONICLES</small></strong></button><span className="context-divider" /><Button variant="quiet" className="campaign-trigger" onClick={() => { if (guard()) { setDirty(false); setChoosing(true); } }}>{campaign?.name ?? "Deine Kampagnen"}<ChevronDown size={15} /></Button><div className="context-right"><span className="user-name">{me.displayName}</span><Button variant="quiet" aria-label="Zugang verwalten" onClick={() => navigate("account")}><Settings size={18} /></Button><Button variant="quiet" aria-label="Abmelden" disabled={task.busy} onClick={() => { if (guard()) void task.run(async () => { await api("/api/logout", { method: "POST" }); setMe(null); setDirty(false); setCampaignId(""); }); }}><LogOut size={18} /></Button></div></header>
    <div className="app-main"><nav className="rail" aria-label="Bereiche">{stages.filter(item => item.id !== "schmiede" || campaign?.role === "leitung").map(({ id, label, icon: Icon }) => <button key={id} className={stage === id && !choosing ? "rail-item active" : "rail-item"} aria-current={stage === id && !choosing ? "page" : undefined} onClick={() => navigate(id)} disabled={!campaign}><Icon size={21} /><span>{label}</span></button>)}</nav>
      <main id="main-content" className="main-stage" tabIndex={-1}>{task.error ? <Notice error>{task.error}</Notice> : null}
        {stage === "account" && !choosing ? <Account me={me} onSessionChanged={refreshSession} /> : campaigns.loading ? <Loading text="Kampagnen werden geladen …" /> : campaigns.error ? <Notice error>{campaigns.error}</Notice> : choosing || !campaign ? <section className="page-content campaign-page"><p className="eyebrow">Deine Geschichten</p><h1>Welches Kapitel schlagen wir auf?</h1><p className="muted">Jede Runde hat ihre Welt. Hier beginnt eure nächste Geschichte.</p>
        ...
        </section> : stage === "wiki" ? <Wiki key={campaign.id} campaign={campaign} onDirty={setDirty} liveRevision={live.revision} onComposeLetter={composeLetter} onOpenDoors={openDoors} /> : stage === "runde" ? <Round key={campaign.id} campaign={campaign} /> : stage === "atlas" ? <Suspense fallback={<Loading text="Atlas wird geöffnet …" />}><AtlasView campaignId={campaign.id} role={campaign.role} onOpenEntry={openEntry} /></Suspense> : stage === "tisch" ? <Suspense fallback={<Loading text="Tisch wird geöffnet …" />}><TableView key={campaign.id} openDoor={doorRequest} liveRevision={live.revision} campaign={campaign} userId={me.userId} onOpenEntry={openEntry} onDirty={setDirty} /></Suspense> : stage === "woche" ? <Suspense fallback={<Loading text="Woche wird geöffnet …" />}><WeekView key={campaign.id} campaign={campaign} userId={me.userId} liveRevision={live.revision} onDirty={setDirty} onOpenEntry={openEntry} onOpenDoors={openDoors} composeSeed={composeSeed} /></Suspense> : stage === "schmiede" ? campaign.role === "leitung" ? <Suspense fallback={<Loading text="Schmiede wird geöffnet …" />}><RuleForge key={campaign.id} campaign={campaign} authorName={me.displayName} onDirty={setDirty} /></Suspense> : <EmptyState title="Die Schmiede gehört der Spielleitung.">Die aktiven Regeln findest du am Tisch.</EmptyState> : stage === "kanal" ? <Suspense fallback={<Loading text="Kanal wird geöffnet …" />}><ChannelView key={campaign.id} campaign={campaign} userId={me.userId} live={live} onDirty={setDirty} /></Suspense> : null}
