...
import { BookOpen, CalendarDays, Hammer, ChevronDown, Compass, Dice6, Home, LogOut, MessageSquare, Plus, Settings, Users } from "lucide-react";
...
type Stage = "heute" | "wiki" | "atlas" | "tisch" | "kanal" | "woche" | "schmiede" | "runde" | "account";
const stages: { id: Stage; label: string; icon: typeof BookOpen }[] = [{ id: "heute", label: "Heute", icon: Home }, { id: "wiki", label: "Chronik", icon: BookOpen }, { id: "atlas", label: "Atlas", icon: Compass }, { id: "tisch", label: "Tisch", icon: Dice6 }, { id: "kanal", label: "Kanal", icon: MessageSquare }, { id: "woche", label: "Woche", icon: CalendarDays }, { id: "schmiede", label: "Schmiede", icon: Hammer }, { id: "runde", label: "Runde", icon: Users }];
...
export function App() {
  ...
  const [stage, setStage] = useState<Stage>(() => { const value = new URLSearchParams(location.search).get("stage"); return value === "heute" || value === "atlas" || value === "runde" || value === "account" || value === "tisch" || value === "kanal" || value === "woche" || value === "schmiede" ? value : "wiki"; });
  ...
  const chooseCampaign = (id: string) => {
    ...
    const url = new URL(location.href); url.searchParams.set("campaign", id); url.searchParams.delete("entry"); url.searchParams.set("stage", "heute"); window.history.replaceState(null, "", url);
  };
