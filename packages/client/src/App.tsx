...
import { Button, EmptyState, Loading, Notice, UiHost } from "@chronicle/ui";
...
import { ContextBanner } from "./features/PixelBanner";
...
export function App() {
  ...
  const [tableRequest, setTableRequest] = useState<{ tab: TableTab; request: number; actor?: string }>();
  ...
  // Woher der Weg in die Einstellungen kam. Ohne das ist der Rueckweg geraten: bei offener
  // Kampagne zurueck in den Bereich, ohne Kampagne in die Kampagnenwahl.
  const [accountReturn, setAccountReturn] = useState<Stage | null>(null);
  // Ein `?join=` in der Adresse las bisher nur die Anmeldeseite — und die wird nur gezeigt,
  // solange NIEMAND angemeldet ist. Wer als angemeldeter Spieler eine Einladung anklickte, sah
  // seine gewohnte App und sonst nichts; der Link lief ins Leere. Jetzt wird er aufgehoben und
  // im eigenen Bereich erklaert.
  const [joinCode] = useState(() => new URLSearchParams(location.search).get("join") ?? "");
  ...
  const navigate = (next: Stage, target?: { forge?: ForgeSection; tab?: TableTab; actor?: string }) => {
    ...
    if (target?.tab) { url.searchParams.set("tab", target.tab); setTableRequest(previous => ({ tab: target.tab!, request: (previous?.request ?? 0) + 1, ...(target.actor ? { actor: target.actor } : {}) })); }
    else setTableRequest(undefined);
    window.history.replaceState(null, "", url);
  };
  /**
   * Der Weg in die Einstellungen und, wichtiger, der Weg zurueck.
   *
   * Vorher gab es hier nur `navigate("account")`. Der Rueckweg lag allein auf der Wortmarke und
   * dem Kampagnenknopf in der Kopfleiste — beide sind nicht als Ausgang beschriftet. Und ohne
   * geoeffnete Kampagne ist die gesamte Bereichsleiste gesperrt: die Seite las sich als Sackgasse.
   */
  const oeffneKonto = () => { if (stage !== "account") setAccountReturn(choosing ? null : stage); navigate("account"); };
  const schliesseKonto = () => {
    if (!guard()) return;
    setDirty(false); setMenuOpen(false);
    const ziel = accountReturn && campaign ? accountReturn : null;
    setAccountReturn(null);
    if (ziel) { navigate(ziel); return; }
    // Ohne Kampagne fuehrt der Ausgang in die Kampagnenwahl — der einzige Ort, an dem es
    // ueberhaupt weitergeht.
    setChoosing(true); setStage("heute");
    const url = new URL(location.href); url.searchParams.set("stage", "heute"); window.history.replaceState(null, "", url);
  };
  // Escape schliesst die Einstellungen. Bewusst ohne Abhaengigkeitsliste: `schliesseKonto` liest
  ...
  /** Abmelden. Der Code in der Adresse bleibt stehen: die Anmeldeseite liest ihn dort. */
  const abmelden = () => { if (!guard()) return; void task.run(async () => {
    await api("/api/logout", { method: "POST" }); setMe(null); setDirty(false); setCampaignId("");
  }); };
  const openForge = (section: ForgeSection) => navigate("schmiede", { forge: section });
  const openTable = (tab: TableTab, actor?: string) => navigate("tisch", { tab, ...(actor ? { actor } : {}) });
