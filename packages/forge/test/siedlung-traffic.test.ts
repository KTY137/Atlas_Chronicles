...
const generate=(verkehr?:RoadPlan)=>erzeugeSiedlung({keim:"roads-regression",optionen:{standort:"ebene",relief:0,...(verkehr?{verkehr}:{})}},pack);
describe("roads integrated into the canonical town generator",()=>{
 ...
 it("keeps ordinary zone-only v9 identities when an empty road plan is supplied",()=>{
   ...
   const first=erzeugeSiedlung({keim:"roads-zones",optionen:{planung}},pack);
   expect(first.version).toBe("9");
   expect(erzeugeSiedlung({keim:"roads-zones",optionen:{planung,verkehr:{...traffic,knoten:[],verbindungen:[]}}},pack)).toEqual(first);
 });
