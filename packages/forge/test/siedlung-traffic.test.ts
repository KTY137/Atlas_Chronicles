...
// Gegenwart und Sci-Fi sind seit Teil 2 (2026-09-23-stadt-zukunft) Version 12, mit und ohne Plan; Fantasy ist v11 (unten).
// Der Keim hängt an der Version: v12 würfelt ein anderes Gelände, auf dem der alte Keim den Markt in einen See legte.
const generate=(verkehr?:RoadPlan,setting:"gegenwart"|"fantasy"="gegenwart")=>erzeugeSiedlung({keim:setting==="fantasy"?"roads-regression":"roads-regression:2",optionen:{standort:"ebene",relief:0,setting,...(verkehr?{verkehr}:{})}},pack);
describe("roads integrated into the canonical town generator",()=>{
 ...
 it("emits ordinary editable regions and a versioned full constraint vector",()=>{const m=generate(traffic);expect(m.version).toBe("12");expect(m.keim.optionen.verkehr).toEqual(traffic);expect(m.bericht.verkehr!.routes.every(r=>r.status==="gebaut")).toBe(true);expect(m.bericht.verkehr!.unreachableNodes).toEqual([]);expect(()=>parseTacticalMapDocument(m.karte)).not.toThrow();expect(()=>parseTacticalCartography(m.cartography,m.karte)).not.toThrow();expect(m.cartography.regions.some(r=>r.role==="road"&&r.material==="square")).toBe(true);});
 ...
 it("keeps ordinary zone-only identities when an empty road plan is supplied",()=>{
