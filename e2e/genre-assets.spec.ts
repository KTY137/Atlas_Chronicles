...
test("300 genre assets are reachable, filterable and persist as editable map artwork", async ({ browser }, testInfo) => {
  ...
  try {
    ...
    const previewed = page.waitForResponse(response => response.url().endsWith("/tactical/generate/preview") && response.request().method() === "POST");
    await workshop.getByRole("button", { name: "Vorschau", exact: true }).click();
    const previewResponse = await previewed; expect(previewResponse.ok()).toBe(true);
    const preview = await previewResponse.json();
    await expect(workshop.locator('.map-workshop-preview .tactical-canvas')).toHaveAttribute("data-canvas-ready", "true");
    ...
    const response = await generated; expect(response.ok()).toBe(true); const savedGeneration = await response.json();
    expect(savedGeneration.keimHash).toBe(preview.keimHash); const mapId = savedGeneration.ack.subjectId;
